...
df <- tibble(croot = seq(1000)) |> 
  mutate(volume_density = croot / (par_rootmorph$rho * par_rootmorph$zroot),
         nup = calc_nup(croot, ninorg = 0.2, par_fnup, par_rootmorph)) |> 
  mutate(nup_fraction = nup/0.2)

df |> 
  ggplot(aes(croot, nup_fraction)) +
  geom_line()

df |> 
  ggplot(aes(volume_density, nup_fraction)) +
  geom_line()
```
...
df <- tibble(ninorg = seq(0.1, 50, by = 0.01)) |> 
  mutate(nup = calc_nup(croot = 500, ninorg, par_fnup, par_rootmorph)) |> 
