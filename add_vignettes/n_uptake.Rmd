...
gg_simpl <- ggplot(df, aes(croot, ninorg)) + 
  ...
  geom_contour(aes(z = fnup), color = "grey50") +
  scale_fill_scico(palette = 'roma', 
                   name = expression(paste("(gN m"^-2,"d"^-1,")"))) +
  theme_classic() +
  labs(title = "N uptake", 
       x = expression(paste("Root C (gC m"^-2,")")), 
       y = expression(paste("Inorganic soil N (gN m"^-2,")")))
gg_simpl
```
...
ggplot(df, aes(croot, ninorg)) + 
  geom_tile(aes(fill = fnup)) +
  geom_contour(aes(z = fnup), color = "grey50", linewidth = 0.2) +
  scale_fill_scico(palette = 'roma', 
                   name = expression(paste("gN m"^-2,"d"^-1))) +
  theme_classic() +
  labs(title = "N uptake", x = "Croot", y = "Ninorg") +
...
ggsave(here::here("fig/n_uptake.pdf"), width = 5, height = 3)
```
