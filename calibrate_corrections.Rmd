...
We use only northern hemisphere sites that belong to the Koeppen-Geiger classification Cfa, Cfb, and Dfb. 
...
#               dplyr::filter( koeppen_code %in% c("Dfb", "Cfb", "Cfa" ) & lat > 0.0 ) %>%
...
## Calibrate the `temp_ramp_edge` parameter.

First run a new model setup function call with reduced number of sites and different output path.
...
Run SOFUN with calibrated parameters for the subset of sites chosen for calibration.
