...
getSetup <- function(x) {
  classes <- class(x)
  if (any(c("mcmcSampler", "smcSampler") %in% classes)) {
    x$setup
  } else if (any(c("mcmcSamplerList", "smcSamplerList") %in% classes)) {
    x[[1]]$setup
  } else {
    stop("Can not get setup from x")
  }
}
...
get_runtime <- function(par_calib) { # function(settings_calib){
  total_time_secs <- sum(unlist(lapply(
    par_calib$mod,
    function(curr_chain) {
      curr_chain$settings$runtime[["elapsed"]]
    }
  )))
...
get_settings_str <- function(par_calib) { # function(settings_calib){
  ...
  nrInternalChains <- lapply(individual_chains, function(curr_chain) {
    curr_chain$settings$nrChains
  }) |>
    unlist() |>
    unique() |>
    paste0(collapse = "-")
  nrIterations <- lapply(individual_chains, function(curr_chain) {
    curr_chain$settings$iterations
  }) |>
    unlist() |>
    unique() |>
    paste0(collapse = "-")
  nrBurnin <- lapply(individual_chains, function(curr_chain) {
    curr_chain$settings$burnin
  }) |>
    unlist() |>
    unique() |>
    paste0(collapse = "-")
  sampler_name <- lapply(individual_chains, function(curr_chain) {
    curr_chain$settings$sampler
  }) |>
    unlist() |>
    unique() |>
    paste0(collapse = "-")
