...
```{r}
calc_net_assim <- function( par, args, iabs, kphio, a_unitcost, maximize=FALSE, return_chi=FALSE ){
  ...
  if (class(root_ci)=="try-error"){
    ...
    # ## Or something equivalent to the P-model: INDEED, THIS IS IT!!!!
    # net_assim <- -(cost_transp + cost_vcmax) / a_c
      
    ## At least this here would be intuitive:
...
out_numerical <- calc_net_assim( par, args, iabs=(fapar * ppfd), kphio = 0.05, a_unitcost=0.0003 )
print(out_numerical)
print( calc_net_assim( par, args, iabs=(fapar * ppfd), kphio = 0.05, a_unitcost=0.0003, return_chi = TRUE ) )
print(out_analytical$chi)
```
...
<!-- sapply( seq(out_analytical$vcmax * 0.10, out_analytical$vcmax * 10, length.out = 100), FUN = function(x) calc_net_assim( c(x, out_analytical$gs), args, iabs=(fapar * ppfd), kphio = 0.05, a_unitcost = 0.00045 ) ) %>% plot(type="l", xlab="Vcmax", ylab="net A") -->
...
<!-- library(dplyr) -->
<!-- ## a_unitcost is searched by hand -->
<!-- sapply( seq(out_analytical$gs * 0.10, out_analytical$gs * 10, length.out = 100), FUN = function(x) calc_net_assim( c(out_analytical$vcmax, x), args, iabs=(fapar * ppfd), kphio = 0.05, a_unitcost = 0.00045 ) ) %>% plot(type="l", xlab="gs", ylab="net A") -->
<!-- abline(v=out_analytical$vcmax, lty=3) -->
<!-- ``` -->
...
print( paste( "chi from gs/Vcmax optimization:", calc_net_assim( par = out_optim$par, args = c( out_analytical$kmm, out_analytical$gammastar, out_analytical$ns_star, out_analytical$ca, vpd, 146.0 ), iabs = (fapar * ppfd), kphio = 0.05, a_unitcost=0.0003, return_chi = TRUE ) ))
...
setzero <- function( x_cost, chi_target, out_analytical, fapar, ppfd, vpd ){
  ...
  chi_numerical <- calc_net_assim( out_optim$par, args, iabs=(fapar * ppfd), kphio = 0.05, a_unitcost=x_cost, return_chi = TRUE )
...
out_optim <- optimr::optimr(
  ...
  iabs       = (fapar * ppfd), 
  kphio      = 0.05,
  a_unitcost = unitcost_root$par,
  method     = "L-BFGS-B",
  maximize   = TRUE,
  control    = list( maxit = 1000 )
  )
print( paste( "chi from gs/Vcmax optimization:", calc_net_assim( par = out_optim$par, args = c( out_analytical$kmm, out_analytical$gammastar, out_analytical$ns_star, out_analytical$ca, vpd, 146.0 ), iabs = (fapar * ppfd), kphio = 0.05, a_unitcost=unitcost_root$par, return_chi = TRUE ) ))
print(paste("... this should be identical to ", out_analytical$chi))
```
Yes!!!
...
len_vcmax <- 50
len_gs <- 50
vcmax_vec <- lseq(out_analytical$vcmax*0.0001, out_analytical$vcmax*20, length.out = len_vcmax)
gs_vec <- lseq(out_analytical$gs*0.001, out_analytical$gs*30, length.out = len_gs)
...
```
The reason why the red and green dots are not exactly on top of each other is because the optimisation (red) returns $A_{n}=A_c - transpcost - vcmaxcost$, while the analytical (green point) returns $A_n = min(A_j, A_c) - transpcost - vcmaxcost$.
