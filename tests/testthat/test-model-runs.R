...
test_that("Regression tests run_biomee_f_bysite()", {
  ...
  ref_BiomeE_Pmodel_odt_yr1 <- tibble(
    ...
    Tc        = c(273.533660888672,271.508483886719,290.345184326172,271.626403808594,273.387603759766), # TODO: this should be renamed Tk since it is in Kelvin (and names in default output and tests modified)
  ...
  ref_BiomeE_gsLeun_odt_yr1 <- tibble(
    year      = c(1,1,1,1,1),
    doy       = c(1,2,180,364,365),
    Tc        = c(273.533660888672,271.508483886719,290.34521484375,271.626373291016,273.387603759766), # TODO: this should be renamed Tk since it is in Kelvin (and names in default output and tests modified)
