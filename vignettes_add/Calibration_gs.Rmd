...
ddf_obs <- data.frame(
    ...
    targets_obs = c(mean_annual_gpp, max_annual_lai, densityLAE, BiomassLAE,LAE_size_dist$nTrees[1],LAE_size_dist$nTrees[2],LAE_size_dist$nTrees[3],LAE_size_dist$nTrees[4],LAE_size_dist$nTrees[5])
    ) 

ddf_obs2 <- data.frame(
    variables = c("dbh_c1","dbh_c2","dbh_c3","dbh_c4","dbh_c5"),
    targets_obs = c(LAE_size_dist$nTrees[1],LAE_size_dist$nTrees[2],LAE_size_dist$nTrees[3],LAE_size_dist$nTrees[4],LAE_size_dist$nTrees[5])
    ) 
...
df_mod_sizedist <- df_output$data[[1]]$output_annual_cohorts %>%
  ...
  mutate(size_bins = cut(dbh, breaks = c(12,32,52,72,92,112))) %>%
  group_by(size_bins,year) %>% summarise(nTrees=sum(density)) %>% ungroup() %>% group_by(size_bins) %>% summarise(nTrees=mean(nTrees))
ggplot(df_mod_sizedist, aes(size_bins, nTrees)) + geom_col()
...
dff2 <- data.frame(
    variables = c("dbh_c1","dbh_c2","dbh_c3","dbh_c4","dbh_c5"),
    targets_mod = c(df_mod_sizedist$nTrees[1],df_mod_sizedist$nTrees[2],df_mod_sizedist$nTrees[3],df_mod_sizedist$nTrees[4],df_mod_sizedist$nTrees[5])
    ) %>% 
    dplyr::left_join(ddf_obs, by = "variables") %>% 
    mutate(error = targets_mod - targets_obs) %>% 
    mutate(error_rel = error / targets_obs) %>% 
    mutate(mean_abs_diff = mean(abs(error_rel))) 
...
settings_calib_NSC_gs <- list(
  ...
  targetvars          = c("targets_obs"),
  timescale           = list(targets_obs = "y"),
  maxit               = 100, # (5 for gensa) (30 for optimr)
  sitenames           = "CH-Lae",
  metric              = "rmse",
  dir_results         = "./",
  name                = "ORG",
...
settings_calib_GR_gs <- list(
  ...
  targetvars          = c("targets_obs"),
  timescale           = list(targets_obs = "y"),
  maxit               = 15, # (5 for gensa) (30 for optimr)
  sitenames           = "CH-Lae",
  metric              = "rmse",
  dir_results         = "./",
  name                = "ORG",
