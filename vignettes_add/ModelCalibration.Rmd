...
PEP725 values
Laegeren is not in the PEP725 data.
```{r}
pep725sitesFagus <- read.csv("/Users/lmarques/data/pep725/PEP725_CH_108_010/PEP725_CH_stations.csv", sep=";")
pep725sitesFagus <- pep725sitesFagus %>% dplyr::filter(NAME=="Höfen")
pep725valuesFagus <- read.csv("/Users/lmarques/data/pep725/PEP725_CH_108_010/PEP725_CH_Fagus(Fagus_sylvatica).csv", sep=";")
pep725valuesFagus <- pep725valuesFagus %>% dplyr::filter(PEP_ID==6758)
ONFagus <- pep725valuesFagus %>% dplyr::filter(BBCH==13)
OFFFagus <- pep725valuesFagus %>% dplyr::filter(BBCH==94)

pep725sitesPicea <- read.csv("/Users/lmarques/data/pep725/PEP725_CH_110_000/PEP725_CH_stations.csv", sep=";")
pep725sitesPicea <- pep725sitesPicea %>% dplyr::filter(NAME=="Höfen")
pep725valuesPicea <- read.csv("/Users/lmarques/data/pep725/PEP725_CH_110_000/PEP725_CH_Picea_abies_(P.excelsa).csv", sep=";")
#pep725valuesPicea <- pep725valuesPicea %>% dplyr::filter(PEP_ID==6758)
#ONFagus <- pep725valuesPicea %>% dplyr::filter(BBCH==13)
#OFFFagus <- pep725valuesPicea %>% dplyr::filter(BBCH==94)
...
Calibration parameters: Quantum Yield Efficiency (kphio), Root:Shoot ratio (phiRL), Plant respiration (tf), 
