...
fig1c_dbh <- ggplot() + 
  ...
  geom_line(data=ea2sa1DBHgl_out_annual_tile, aes(x=year, y=mortality_M, color='+15%'),alpha=.7) +
  geom_line(data=ea3sa1DBHgl_out_annual_tile, aes(x=year, y=mortality_M, color='+30%'),alpha=.7) +
  scale_color_manual("Level of LUE", breaks = c("Control", "+15%", "+30%"), values = c("#F8766D" ,"#619CFF", "#00BA38")) +
  labs(x = "Year", y = expression(paste("Biomass (Kg C ", m^-2, " ", yr^-1, ") "))) + 
  theme_bw() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),legend.position = "bottom",
        axis.text = element_text(size = 10),axis.title = element_text(size = 11),
...
ggplot() + 
  geom_line(data=ea1sa1DBHgl_out_annual_tile, aes(x=year, y=mortality_M, color='Control'),alpha=.7) + 
  geom_line(data=ea2sa1DBHgl_out_annual_tile, aes(x=year, y=mortality_M, color='+15%'),alpha=.7) +
  geom_line(data=ea3sa1DBHgl_out_annual_tile, aes(x=year, y=mortality_M, color='+30%'),alpha=.7) 

_deadtrees + fastSOM + SlowSOM
...
ggarrange(fig1a_dbh,fig1b_dbh,fig1d_dbh,fig1f_dbh,fig1g1_dbh,fig1g2_dbh,fig1h_dbh, ncol = 7, nrow = 1, common.legend = T,legend="bottom")
...
fig2c_dbh <- ggplot() + 
  ...
  scale_color_manual("Level of LUE", breaks = c("300 kgC m-3", "450 kgC m-3", "+30%"), values = c("#00AFBB", "#E7B800", "#FC4E07")) +
  labs(x = "Year", y = expression(paste("Biomass (Kg C ", m^-2, " ", yr^-1, ") "))) + 
  theme_bw() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),legend.position = "bottom",
        axis.text = element_text(size = 10),axis.title = element_text(size = 11),
        legend.text = element_text(size = 10),legend.title = element_text(size = 11),
...
# Calculate the relative change as (Final value - initial value)/initial value
NPP30 <- ea1sb3DBHgl_out_annual_tileMaxAgg %>% dplyr::filter(year>=700&year<=1500) %>% summarise(NPP30=mean(NPP))  #NPP or A_NPP
NPP15 <- ea1sb2DBHgl_out_annual_tileMaxAgg %>% dplyr::filter(year>=700&year<=1500) %>% summarise(NPP15=mean(NPP))  
NPP0  <- ea1sb1DBHgl_out_annual_tileMaxAgg %>% dplyr::filter(year>=700&year<=1500) %>% summarise(NPP0=mean(NPP))
dNPP0_15 <- (NPP15$NPP15 - NPP0$NPP0)/NPP0$NPP0
dNPP15_30 <- (NPP30$NPP30 - NPP15$NPP15)/NPP15$NPP15
dNPP0_30 <- (NPP30$NPP30 - NPP0$NPP0)/NPP0$NPP0

L30 <- ea1sb3DBHgl_out_annual_tileMaxAgg %>% dplyr::filter(year>=700&year<=1500) %>% summarise(L30=mean(MaxAge)) 
L15 <- ea1sb2DBHgl_out_annual_tileMaxAgg %>% dplyr::filter(year>=700&year<=1500) %>% summarise(L15=mean(MaxAge))
L0  <- ea1sb1DBHgl_out_annual_tileMaxAgg %>% dplyr::filter(year>=700&year<=1500) %>% summarise(L0=mean(MaxAge))
dL0_15 <- (L15$L15 - L0$L0)/L0$L0
dL15_30 <- (L30$L30 - L15$L15)/L15$L15
dL0_30 <- (L30$L30 - L0$L0)/L0$L0
