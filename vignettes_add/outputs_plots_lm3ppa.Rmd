...
ggplot() + 
  geom_line(data=ea1sa1DBHgl_out_annual_tile, aes(x=year, y=NPP, color='Control'),alpha=.7) + 
  geom_line(data=ea2sa1DBHgl_out_annual_tile, aes(x=year, y=NPP, color='+15'),alpha=.7) +
  geom_line(data=ea3sa1DBHgl_out_annual_tile, aes(x=year, y=NPP, color='+30'),alpha=.7)
...
ggplot() + 
  geom_line(data=ea1sa1NSCp3gl_out_annual_tile, aes(x=year, y=plantC, color='Control'),alpha=.7) + 
  geom_line(data=ea2sa1NSCp3gl_out_annual_tile, aes(x=year, y=plantC, color='+15'),alpha=.7) +
  geom_line(data=ea3sa1NSCp3gl_out_annual_tile, aes(x=year, y=plantC, color='+30'),alpha=.7)
...
#### DBH
```{r}
fig1a_dbh <- ggplot(data.frame(x = c(0, 1.05)), aes(x)) + 
  stat_function(fun = ~ 0.1*.x ^ 1.5, col="#009E73") +  
  stat_function(fun = ~ 0.1*.x ^ 2.5, col="#0072B2") + 
  stat_function(fun = ~ 0.1*.x ^ 4.0, col="#D55E00") + 
  labs(x='DBH', y='m') +
...
fig1a_nsc <- ggplot(data.frame(x = c(0, 5)), aes(x)) + 
  stat_function(fun = ~ 0.1/(1+(exp(2*(.x-2.5)))), col="#009E73") +
  stat_function(fun = ~ 0.1/(1+(exp(3*(.x-2.5)))), col="#0072B2") +
  stat_function(fun = ~ 0.1/(1+(exp(5*(.x-2.5)))), col="#D55E00") +
...
fig1a_gr <- ggplot(data.frame(x = c(0, 20)), aes(x)) + 
  stat_function(fun = ~ 0.125/(1+(exp(-0.5*(.x-10)))), col="#009E73") +
  stat_function(fun = ~ 0.125/(1+(exp(-0.8*(.x-10)))), col="#0072B2") +
  stat_function(fun = ~ 0.125/(1+(exp(-1.4*(.x-10)))), col="#D55E00") +
  labs(x='GR', y='m',title= "q)") +theme_bw() +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),legend.position = "bottom",
        axis.text = element_text(size = 10),axis.title = element_text(size = 11),
        legend.text = element_text(size = 10),legend.title = element_text(size = 11),
...
plot(data_DBH_ea1p1$NPP ~ data_DBH_ea1p1$QMD)
data_DBH_ea1p1$logNPP <- log(data_DBH_ea1p1$NPP)
FitRes = lm(logNPP ~ scale(QMD), data = data_DBH_ea1p1, na.action = "na.exclude")
summary(FitRes)
Res_NPP <- residuals(FitRes)
Fitted_NPP <- fitted(FitRes)
data_DBH_ea1p1$Res_NPP <- Res_NPP

par(mfrow=c(2,2))
plot(Res_NPP ~ Fitted_NPP, xlab="Fitted values", ylab="Residuals", main="Residuals vs. fitted")
abline(h=0) ## Homocedasticity
plot(data_DBH_ea1p1$logNPP ~ Fitted_NPP, xlab="FittedG", ylab="NPP", main = "xyplot")
abline(0, 1) 
hist(Res_NPP, main="Histogram of residuals", xlab="Residuals") ## Normality
boxplot(Res_NPP,ylab = "Residuals")

max(data_DBH_ea1p1$NPP)
min(data_DBH_ea1p1$Res_NPP)
...
# Residuals G ~ QMD
plot(data_DBH_p1Den$NPP ~ data_DBH_p1Den$QMD)
data_DBH_p1Den$logNPP <- log(data_DBH_p1Den$NPP)

FitRes = lm(logNPP ~ scale(QMD), data = data_DBH_p1Den, na.action = "na.exclude")
summary(FitRes)
Res_NPP <- residuals(FitRes)
Fitted_NPP <- fitted(FitRes)
...
plot(Res_NPP ~ Fitted_NPP, xlab="Fitted values", ylab="Residuals", main="Residuals vs. fitted")
abline(h=0) ## Homocedasticity
plot(data_DBH_p1Den$logNPP ~ FittedG, xlab="FittedG", ylab="NPP", main = "xyplot")
abline(0, 1) 
hist(Res_NPP, main="Histogram of residuals", xlab="Residuals") ## Normality
boxplot(Res_NPP,ylab = "Residuals")
...
Fit_QMDInter = lm(logDensity12 ~ scale(logQMD) * scale(NPP), data = data_DBH_p1Den, na.action = "na.exclude")
Fit_QMDNoInter = lm(logDensity12 ~ scale(logQMD) + scale(NPP), data = data_DBH_p1Den, na.action = "na.exclude")
...
fig2a_dbh <- plot_model(Fit_QMDInter, type = "pred",show.data=TRUE, dot.size=1.5,
                         terms = c("logQMD","Res_NPP[-0.1,0,0.1]"),title = "",
...
fig2b_dbh <- plot_model(Fit_QMDInter, type = "pred",show.data=TRUE, dot.size=1.5,
...
plot(allEffects(Fit_QMDNoInter))
fig2c_dbh <- plot_model(Fit_QMDInter, type = "pred",show.data=TRUE, dot.size=1.5,
                         terms = c("logQMD","LUE"),title = "",
                         axis.title = c("Quadratic Mean Diameter (QMD)","Stand density (N)"),legend.title = "LUE",
                         colors = c("#FC4E07", "#00AFBB", "#E7B800")) + 
...
ea1sa1NSCp1gl_out_annual_tile <- ea1sa1NSCp1gl_out_annual_tile %>% 
  mutate(Mortality = "NSC") %>% 
  mutate(Parameter = "p1") %>% 
  mutate(LUE = "Control")

data_NSC_ea1p1 <- ea1sa1NSCp1gl_out_annual_tile %>% filter(year>=750) %>% mutate(QMD_bins = cut(QMD, breaks = 30)) 
sort(unique(data_NSC_ea1p1$QMD_bins))

# Select from each QMD bins the number of plots with higher density
# Includes a sensitivity analysis using quantiles from 0.5 to 0.9
valueQuantile = 0.75
quantileX <- data_NSC_ea1p1 %>% group_by(QMD_bins) %>% summarise(quantile(Density12, c(valueQuantile))) 
max(quantileX$`quantile(Density12, c(valueQuantile))`)
data_NSC_ea1p1 <- data_NSC_ea1p1 %>% left_join(quantileX)
data_NSC_ea1p1Den <- data_NSC_ea1p1 %>% filter(Density12>=`quantile(Density12, c(valueQuantile))`)
data_NSC_ea1p1Rest <- data_NSC_ea1p1 %>% filter(Density12<`quantile(Density12, c(valueQuantile))`)

ea2sa1NSCp1gl_out_annual_tile <- ea2sa1NSCp1gl_out_annual_tile %>%
  mutate(Mortality = "NSC") %>% 
  mutate(Parameter = "p1") %>% 
  mutate(LUE = "+15%")

data_NSC_ea2p1 <- ea2sa1NSCp1gl_out_annual_tile %>% filter(year>=750) %>% mutate(QMD_bins = cut(QMD, breaks = 30)) 
sort(unique(data_NSC_ea2p1$QMD_bins))

# Select from each QMD bins the number of plots with higher density
# Includes a sensitivity analysis using quantiles from 0.5 to 0.9
valueQuantile = 0.75
quantileX <- data_NSC_ea2p1 %>% group_by(QMD_bins) %>% summarise(quantile(Density12, c(valueQuantile))) 
max(quantileX$`quantile(Density12, c(valueQuantile))`)
data_NSC_ea2p1 <- data_NSC_ea2p1 %>% left_join(quantileX)
data_NSC_ea2p1Den <- data_NSC_ea2p1 %>% filter(Density12>=`quantile(Density12, c(valueQuantile))`)
data_NSC_ea2p1Rest <- data_NSC_ea2p1 %>% filter(Density12<`quantile(Density12, c(valueQuantile))`)

ea3sa1NSCp1gl_out_annual_tile <- ea3sa1NSCp1gl_out_annual_tile %>% 
  mutate(Mortality = "NSC") %>% 
  mutate(Parameter = "p1") %>% 
  mutate(LUE = "+30%")

data_NSC_ea3p1 <- ea3sa1NSCp1gl_out_annual_tile %>% filter(year>=750) %>% mutate(QMD_bins = cut(QMD, breaks = 30)) 
sort(unique(data_NSC_ea3p1$QMD_bins))

# Select from each QMD bins the number of plots with higher density
# Includes a sensitivity analysis using quantiles from 0.5 to 0.9
valueQuantile = 0.75
quantileX <- data_NSC_ea3p1 %>% group_by(QMD_bins) %>% summarise(quantile(Density12, c(valueQuantile))) 
max(quantileX$`quantile(Density12, c(valueQuantile))`)
data_NSC_ea3p1 <- data_NSC_ea3p1 %>% left_join(quantileX)
data_NSC_ea3p1Den <- data_NSC_ea3p1 %>% filter(Density12>=`quantile(Density12, c(valueQuantile))`)
data_NSC_ea3p1Rest <- data_NSC_ea3p1 %>% filter(Density12<`quantile(Density12, c(valueQuantile))`)

data_NSC_p1 <- rbind(data_NSC_ea1p1, data_NSC_ea2p1, data_NSC_ea3p1) 
data_NSC_p1Den <- rbind(data_NSC_ea1p1Den, data_NSC_ea2p1Den, data_NSC_ea3p1Den) 
data_NSC_p1Rest <- rbind(data_NSC_ea1p1Rest, data_NSC_ea2p1Rest, data_NSC_ea3p1Rest) 

data_NSC_p1Den <- data_NSC_p1Den %>% mutate(logDensity12=log(Density12),logQMD=log(QMD))
data_NSC_p1Rest <- data_NSC_p1Rest %>% mutate(logDensity12=log(Density12),logQMD=log(QMD))

sort(unique(data_NSC_p1$QMD_bins))

ggplot() + 
  geom_point(data = data_NSC_p1, aes(x = log(QMD), y = log(Density12)), color="blue") +
  geom_point(data = data_NSC_p1Den, aes(x = log(QMD), y = log(Density12)), color="red") 

### 75th percentile
Fit_QMDInter = lm(logDensity12 ~ scale(logQMD) * LUE, data = data_NSC_p1Den, na.action = "na.exclude")
Fit_QMDNoInter = lm(logDensity12 ~ scale(logQMD) + LUE, data = data_NSC_p1Den, na.action = "na.exclude")
AICc(Fit_QMDInter,Fit_QMDNoInter)
summary(Fit_QMDInter)
summary(Fit_QMDNoInter)
plot(allEffects(Fit_QMDInter))
fig2d_nsc <- plot_model(Fit_QMDInter, type = "pred",show.data=TRUE, dot.size=1.5,
                         terms = c("logQMD","LUE"),title = "",
                         axis.title = c("Quadratic Mean Diameter (QMD)","Stand density (N)"),legend.title = "LUE",
                         colors = c("#FC4E07", "#00AFBB", "#E7B800")) + 
  #geom_point(data = data_NSC_p1Rest, aes(x = logQMD, y = logDensity12), alpha=0.2, size = 1,shape = 18, inherit.aes = FALSE) + 
  theme_bw() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),legend.position = "bottom",
        axis.text = element_text(size = 10),axis.title = element_text(size = 11),
        legend.text = element_text(size = 10),legend.title = element_text(size = 11),
        plot.title = element_text(size = 11)) 
...
plot(ResG ~ FittedG, xlab="Fitted values", ylab="Residuals", main="Residuals vs. fitted")
abline(h=0) ## Homocedasticity
plot(data_NSC_p1Den$logDensity12 ~ FittedG, xlab="FittedG", ylab="TreesPerHectareAHC1_2", main = "xyplot")
abline(0, 1) 
hist(ResG, main="Histogram of residuals", xlab="Residuals") ## Normality
boxplot(ResG,ylab = "Residuals")
...
ea1sa1NSCp2gl_out_annual_tile <- ea1sa1NSCp2gl_out_annual_tile %>% 
  mutate(Mortality = "NSC") %>% 
  mutate(Parameter = "p2") %>% 
  mutate(LUE = "Control")

data_NSC_ea1p2 <- ea1sa1NSCp2gl_out_annual_tile %>% filter(year>=750) %>% mutate(QMD_bins = cut(QMD, breaks = 30)) 
sort(unique(data_NSC_ea1p2$QMD_bins))

# Select from each QMD bins the number of plots with higher density
# Includes a sensitivity analysis using quantiles from 0.5 to 0.9
valueQuantile = 0.75
quantileX <- data_NSC_ea1p2 %>% group_by(QMD_bins) %>% summarise(quantile(Density12, c(valueQuantile))) 
max(quantileX$`quantile(Density12, c(valueQuantile))`)
data_NSC_ea1p2 <- data_NSC_ea1p2 %>% left_join(quantileX)
data_NSC_ea1p2Den <- data_NSC_ea1p2 %>% filter(Density12>=`quantile(Density12, c(valueQuantile))`)
data_NSC_ea1p2Rest <- data_NSC_ea1p2 %>% filter(Density12<`quantile(Density12, c(valueQuantile))`)

ea2sa1NSCp2gl_out_annual_tile <- ea2sa1NSCp2gl_out_annual_tile %>%
  mutate(Mortality = "NSC") %>% 
  mutate(Parameter = "p2") %>% 
  mutate(LUE = "+15%")

data_NSC_ea2p2 <- ea2sa1NSCp2gl_out_annual_tile %>% filter(year>=750) %>% mutate(QMD_bins = cut(QMD, breaks = 30)) 
sort(unique(data_NSC_ea2p2$QMD_bins))

# Select from each QMD bins the number of plots with higher density
# Includes a sensitivity analysis using quantiles from 0.5 to 0.9
valueQuantile = 0.75
quantileX <- data_NSC_ea2p2 %>% group_by(QMD_bins) %>% summarise(quantile(Density12, c(valueQuantile))) 
max(quantileX$`quantile(Density12, c(valueQuantile))`)
data_NSC_ea2p2 <- data_NSC_ea2p2 %>% left_join(quantileX)
data_NSC_ea2p2Den <- data_NSC_ea2p2 %>% filter(Density12>=`quantile(Density12, c(valueQuantile))`)
data_NSC_ea2p2Rest <- data_NSC_ea2p2 %>% filter(Density12<`quantile(Density12, c(valueQuantile))`)

ea3sa1NSCp2gl_out_annual_tile <- ea3sa1NSCp2gl_out_annual_tile %>% 
  mutate(Mortality = "NSC") %>% 
  mutate(Parameter = "p2") %>% 
  mutate(LUE = "+30%")

data_NSC_ea3p2 <- ea3sa1NSCp2gl_out_annual_tile %>% filter(year>=750) %>% mutate(QMD_bins = cut(QMD, breaks = 30)) 
sort(unique(data_NSC_ea3p2$QMD_bins))

# Select from each QMD bins the number of plots with higher density
# Includes a sensitivity analysis using quantiles from 0.5 to 0.9
valueQuantile = 0.75
quantileX <- data_NSC_ea3p2 %>% group_by(QMD_bins) %>% summarise(quantile(Density12, c(valueQuantile))) 
max(quantileX$`quantile(Density12, c(valueQuantile))`)
data_NSC_ea3p2 <- data_NSC_ea3p2 %>% left_join(quantileX)
data_NSC_ea3p2Den <- data_NSC_ea3p2 %>% filter(Density12>=`quantile(Density12, c(valueQuantile))`)
data_NSC_ea3p2Rest <- data_NSC_ea3p2 %>% filter(Density12<`quantile(Density12, c(valueQuantile))`)

data_NSC_p2 <- rbind(data_NSC_ea1p2, data_NSC_ea2p2, data_NSC_ea3p2) 
data_NSC_p2Den <- rbind(data_NSC_ea1p2Den, data_NSC_ea2p2Den, data_NSC_ea3p2Den) 
data_NSC_p2Rest <- rbind(data_NSC_ea1p2Rest, data_NSC_ea2p2Rest, data_NSC_ea3p2Rest) 

data_NSC_p2Den <- data_NSC_p2Den %>% mutate(logDensity12=log(Density12),logQMD=log(QMD))
data_NSC_p2Rest <- data_NSC_p2Rest %>% mutate(logDensity12=log(Density12),logQMD=log(QMD))

sort(unique(data_NSC_p2$QMD_bins))

ggplot() + 
  geom_point(data = data_NSC_p2, aes(x = log(QMD), y = log(Density12)), color="blue") +
  geom_point(data = data_NSC_p2Den, aes(x = log(QMD), y = log(Density12)), color="red") 

### 75th percentile
Fit_QMDInter = lm(logDensity12 ~ scale(logQMD) * LUE, data = data_NSC_p2Den, na.action = "na.exclude")
Fit_QMDNoInter = lm(logDensity12 ~ scale(logQMD) + LUE, data = data_NSC_p2Den, na.action = "na.exclude")
AICc(Fit_QMDInter,Fit_QMDNoInter)
summary(Fit_QMDInter)
summary(Fit_QMDNoInter)
plot(allEffects(Fit_QMDInter))
fig2e_nsc <- plot_model(Fit_QMDInter, type = "pred",show.data=TRUE, dot.size=1.5,
                         terms = c("logQMD","LUE"),title = "",
                         axis.title = c("Quadratic Mean Diameter (QMD)","Stand density (N)"),legend.title = "LUE",
                         colors = c("#FC4E07", "#00AFBB", "#E7B800")) + 
  #geom_point(data = data_NSC_p2Rest, aes(x = logQMD, y = logDensity12), alpha=0.2, size = 1,shape = 18, inherit.aes = FALSE) + 
  theme_bw() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),legend.position = "bottom",
        axis.text = element_text(size = 10),axis.title = element_text(size = 11),
        legend.text = element_text(size = 10),legend.title = element_text(size = 11),
        plot.title = element_text(size = 11)) 
...
plot(ResG ~ FittedG, xlab="Fitted values", ylab="Residuals", main="Residuals vs. fitted")
abline(h=0) ## Homocedasticity
plot(data_NSC_p2Den$logDensity12 ~ FittedG, xlab="FittedG", ylab="TreesPerHectareAHC1_2", main = "xyplot")
abline(0, 1) 
hist(ResG, main="Histogram of residuals", xlab="Residuals") ## Normality
boxplot(ResG,ylab = "Residuals")
...
ggplot() + 
  ...
  geom_point(data=subset(ea2sa1NSCp2gl_out_annual_tile, year>=750), aes(x=log(QMD), y=log(Density12)),alpha=.5,size=1,col="grey") + 
  geom_point(data=subset(ea3sa1NSCp2gl_out_annual_tile, year>=750), aes(x=log(QMD), y=log(Density12)),alpha=.5,size=1,col="grey") + 
  geom_smooth(data= subset(ea1sa1NSCp2gl_out_annual_tile, year>=750), aes(x=log(QMD), y=log(Density12), linetype='Control'),col="#009E73",
              method = "lm",fullrange = T) + 
  geom_smooth(data= subset(ea2sa1NSCp2gl_out_annual_tile, year>=750), aes(x=log(QMD), y=log(Density12), linetype='+15%'),col="#009E73",
              method = "lm",fullrange = T) +
  geom_smooth(data= subset(ea3sa1NSCp2gl_out_annual_tile, year>=750), aes(x=log(QMD), y=log(Density12), linetype='+30%'),col="#009E73",
              method = "lm",fullrange = T) +
  scale_linetype_manual("Level of LUE", breaks = c("Control","+15%", "+30%"), 
                     values = c("dotted","dashed","solid")) +
  labs(x = "Quadratic Mean Diameter (QMD)", y = "Stand density (N)",
...
ea1sa1NSCp3gl_out_annual_tile <- ea1sa1NSCp3gl_out_annual_tile %>% 
  mutate(Mortality = "NSC") %>% 
  mutate(Parameter = "p3") %>% 
  mutate(LUE = "Control")

data_NSC_ea1p3 <- ea1sa1NSCp3gl_out_annual_tile %>% filter(year>=750) %>% mutate(QMD_bins = cut(QMD, breaks = 30)) 
sort(unique(data_NSC_ea1p3$QMD_bins))

# Select from each QMD bins the number of plots with higher density
# Includes a sensitivity analysis using quantiles from 0.5 to 0.9
valueQuantile = 0.75
quantileX <- data_NSC_ea1p3 %>% group_by(QMD_bins) %>% summarise(quantile(Density12, c(valueQuantile))) 
max(quantileX$`quantile(Density12, c(valueQuantile))`)
data_NSC_ea1p3 <- data_NSC_ea1p3 %>% left_join(quantileX)
data_NSC_ea1p3Den <- data_NSC_ea1p3 %>% filter(Density12>=`quantile(Density12, c(valueQuantile))`)
data_NSC_ea1p3Rest <- data_NSC_ea1p3 %>% filter(Density12<`quantile(Density12, c(valueQuantile))`)

ea2sa1NSCp3gl_out_annual_tile <- ea2sa1NSCp3gl_out_annual_tile %>%
  mutate(Mortality = "NSC") %>% 
  mutate(Parameter = "p3") %>% 
  mutate(LUE = "+15%")

data_NSC_ea2p3 <- ea2sa1NSCp3gl_out_annual_tile %>% filter(year>=750) %>% mutate(QMD_bins = cut(QMD, breaks = 30)) 
sort(unique(data_NSC_ea2p3$QMD_bins))

# Select from each QMD bins the number of plots with higher density
# Includes a sensitivity analysis using quantiles from 0.5 to 0.9
valueQuantile = 0.75
quantileX <- data_NSC_ea2p3 %>% group_by(QMD_bins) %>% summarise(quantile(Density12, c(valueQuantile))) 
max(quantileX$`quantile(Density12, c(valueQuantile))`)
data_NSC_ea2p3 <- data_NSC_ea2p3 %>% left_join(quantileX)
data_NSC_ea2p3Den <- data_NSC_ea2p3 %>% filter(Density12>=`quantile(Density12, c(valueQuantile))`)
data_NSC_ea2p3Rest <- data_NSC_ea2p3 %>% filter(Density12<`quantile(Density12, c(valueQuantile))`)

ea3sa1NSCp3gl_out_annual_tile <- ea3sa1NSCp3gl_out_annual_tile %>% 
  mutate(Mortality = "NSC") %>% 
  mutate(Parameter = "p3") %>% 
  mutate(LUE = "+30%")

data_NSC_ea3p3 <- ea3sa1NSCp3gl_out_annual_tile %>% filter(year>=750) %>% mutate(QMD_bins = cut(QMD, breaks = 30)) 
sort(unique(data_NSC_ea3p3$QMD_bins))

# Select from each QMD bins the number of plots with higher density
# Includes a sensitivity analysis using quantiles from 0.5 to 0.9
valueQuantile = 0.75
quantileX <- data_NSC_ea3p3 %>% group_by(QMD_bins) %>% summarise(quantile(Density12, c(valueQuantile))) 
max(quantileX$`quantile(Density12, c(valueQuantile))`)
data_NSC_ea3p3 <- data_NSC_ea3p3 %>% left_join(quantileX)
data_NSC_ea3p3Den <- data_NSC_ea3p3 %>% filter(Density12>=`quantile(Density12, c(valueQuantile))`)
data_NSC_ea3p3Rest <- data_NSC_ea3p3 %>% filter(Density12<`quantile(Density12, c(valueQuantile))`)

data_NSC_p3 <- rbind(data_NSC_ea1p3, data_NSC_ea2p3, data_NSC_ea3p3) 
data_NSC_p3Den <- rbind(data_NSC_ea1p3Den, data_NSC_ea2p3Den, data_NSC_ea3p3Den) 
data_NSC_p3Rest <- rbind(data_NSC_ea1p3Rest, data_NSC_ea2p3Rest, data_NSC_ea3p3Rest) 

data_NSC_p3Den <- data_NSC_p3Den %>% mutate(logDensity12=log(Density12),logQMD=log(QMD))
data_NSC_p3Rest <- data_NSC_p3Rest %>% mutate(logDensity12=log(Density12),logQMD=log(QMD))

sort(unique(data_NSC_p3$QMD_bins))

ggplot() + 
  geom_point(data = data_NSC_p3, aes(x = log(QMD), y = log(Density12)), color="blue") +
  geom_point(data = data_NSC_p3Den, aes(x = log(QMD), y = log(Density12)), color="red") 

### 75th percentile
Fit_QMDInter = lm(logDensity12 ~ scale(logQMD) * LUE, data = data_NSC_p3Den, na.action = "na.exclude")
Fit_QMDNoInter = lm(logDensity12 ~ scale(logQMD) + LUE, data = data_NSC_p3Den, na.action = "na.exclude")
AICc(Fit_QMDInter,Fit_QMDNoInter)
summary(Fit_QMDInter)
summary(Fit_QMDNoInter)
plot(allEffects(Fit_QMDInter))
fig2f_nsc <- plot_model(Fit_QMDInter, type = "pred",show.data=TRUE, dot.size=1.5,
                         terms = c("logQMD","LUE"),title = "",
                         axis.title = c("Quadratic Mean Diameter (QMD)","Stand density (N)"),legend.title = "LUE",
                         colors = c("#FC4E07", "#00AFBB", "#E7B800")) + 
  #geom_point(data = data_NSC_p3Rest, aes(x = logQMD, y = logDensity12), alpha=0.2, size = 1,shape = 18, inherit.aes = FALSE) + 
  theme_bw() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),legend.position = "bottom",
        axis.text = element_text(size = 10),axis.title = element_text(size = 11),
        legend.text = element_text(size = 10),legend.title = element_text(size = 11),
        plot.title = element_text(size = 11)) 
...
plot(ResG ~ FittedG, xlab="Fitted values", ylab="Residuals", main="Residuals vs. fitted")
abline(h=0) ## Homocedasticity
plot(data_NSC_p3Den$logDensity12 ~ FittedG, xlab="FittedG", ylab="TreesPerHectareAHC1_2", main = "xyplot")
abline(0, 1) 
hist(ResG, main="Histogram of residuals", xlab="Residuals") ## Normality
boxplot(ResG,ylab = "Residuals")
...
sort(unique(data_GR_p1$QMD_bins))
...
summary(Fit_QMDNoInter)
plot(allEffects(Fit_QMDInter))
fig2g_gr <- plot_model(Fit_QMDInter, type = "pred",show.data=TRUE, dot.size=1.5,
                         terms = c("logQMD","LUE"),title = "",
                         axis.title = c("Quadratic Mean Diameter (QMD)","Stand density (N)"),legend.title = "LUE",
                         colors = c("#FC4E07", "#00AFBB", "#E7B800")) + 
  #geom_point(data = data_GR_p1Rest, aes(x = logQMD, y = logDensity12), alpha=0.2, size = 1,shape = 18, inherit.aes = FALSE) + 
  theme_bw() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),legend.position = "bottom",
        axis.text = element_text(size = 10),axis.title = element_text(size = 11),
        legend.text = element_text(size = 10),legend.title = element_text(size = 11),
        plot.title = element_text(size = 11)) #+ 
...
sort(unique(data_GR_p2$QMD_bins))
...
summary(Fit_QMDNoInter)
plot(allEffects(Fit_QMDInter))
fig2h_gr <- plot_model(Fit_QMDInter, type = "pred",show.data=TRUE, dot.size=1.5,
                         terms = c("logQMD","LUE"),title = "",
                         axis.title = c("Quadratic Mean Diameter (QMD)","Stand density (N)"),legend.title = "LUE",
                         colors = c("#FC4E07", "#00AFBB", "#E7B800")) + 
...
sort(unique(data_GR_p3$QMD_bins))
...
summary(Fit_QMDNoInter)
plot(allEffects(Fit_QMDInter))
fig2i_gr <- plot_model(Fit_QMDInter, type = "pred",show.data=TRUE, dot.size=1.5,
                         terms = c("logQMD","LUE"),title = "",
                         axis.title = c("Quadratic Mean Diameter (QMD)","Stand density (N)"),legend.title = "LUE",
                         colors = c("#FC4E07", "#00AFBB", "#E7B800")) + 
...
ggsave("~/rsofun/vignettes_add/figure1b.png", width = 12, height = 17, dpi=300)
```
...
ggsave("~/rsofun/vignettes_add/figure2b.png", width = 10, height = 10, dpi=300)
```
...
fig3 <- alldata %>%
  filter(dbh >= 12) %>% 
  ggplot() + geom_line(aes(x=log(dbh), y=log(density), color='x3'),alpha=.5) 
    labs(x = "DBH class (m)", y = expression(paste("Stand density (indiv", " " ,ha^-1, ") "))) +
    scale_fill_discrete(labels = c(`1` = "Control", `2` = "+15%", `3` = "+30%"),name = "Levels of LUE", guide = "legend") + 
    theme_bw() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),legend.position = "bottom") +
  facet_grid(Mortality ~ Parameter) 
...
ggplot() + 
  geom_line(data=subset(ea1sa1DBHp3gl_out_annual_cohorts,dbh>=12), aes(x=dbh, y=density, color='x3'),alpha=.5) +
  geom_line(data=subset(ea1sa1DBHp2gl_out_annual_cohorts,dbh>=12), aes(x=dbh, y=density, color='x2'),alpha=.5) +
  geom_line(data=subset(ea1sa1DBHp1gl_out_annual_cohorts,dbh>=12), aes(x=dbh, y=density, color='x1'),alpha=.5) + 
  scale_color_manual("Parameter value", breaks = c("x1", "x2", "x3"), 
                     values = c("#009E73", "#0072B2", "#D55E00")) +
  labs(x = "dbh", y = "CUE") + 
  theme_bw() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),legend.position = "bottom",
...
fig3 <- alldata %>%
  filter(dbh >= 12) %>%
  ggplot() + geom_line(aes(color=as.factor(LUE), x=dbh, y=density), alpha=.4) +
    labs(x = "DBH (m)", y = "Density") +
    scale_color_discrete(labels = c(`1` = "Control", `2` = "+15%", `3` = "+30%"),name = "Levels of LUE", guide = "legend") + 
    theme_bw() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),legend.position = "bottom") +
  facet_grid(Mortality ~ Parameter) 

ggsave("~/rsofun/vignettes_add/figure3b.png", width = 11, height = 8, dpi=300)
