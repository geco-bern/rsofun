...
FluxForcing <- FluxForcing %>% dplyr::rename(PAR=PPFD_IN,Swdown=SW_IN_F,TEMP=TA_F,SoilT=TS_F_MDS_1,RH=RH,RAIN=P_F,WIND=WS_F,PRESSURE=PA_F,SWC=SWC_F_MDS_1) %>% dplyr::select(-TIMESTAMP_START) %>% 
  mutate(datehour = make_datetime(YEAR, MONTH, DOY, HOUR))  %>% mutate(date = make_datetime(YEAR, MONTH, DOY)) %>%
...
FluxForcing <- FluxForcing %>% 
  ...
  left_join(
    df_co2 %>% 
      tidyr::unnest(data),
    by = c("sitename", "date")
  ) %>% rename(CO2=co2) %>% relocate(CO2, .after = PRESSURE) 
  #group_by(sitename) %>% 
  #tidyr::nest()

FluxForcing <- FluxForcing %>% relocate(co2, .after = PRESSURE) 
```
...
if (params_siml$method_photosynth == "gs_leuning"){
  ...
  FluxForcing <- FluxForcing[,-c(1:3,15)]
}
...
df_drivers2 <- collect_drivers_sofun( 
  siteinfo       = siteinfo,
  meteo          = ddf_meteo, 
