...
ggplot(data.frame(x = c(0, 1.5)), aes(x)) + stat_function(fun = ~ 0.05*exp(2*.x),col="blue") + stat_function(fun = ~ 0.05*exp(3*.x),col="red") +
  labs(x='DBH (m)', y='Deathrate') + theme_bw() + scale_y_continuous(limits = c(0,1))

ggplot(data.frame(x = c(0, 1.5)), aes(x)) + stat_function(fun = ~ 10*(exp(3*(.x-2)))/(1+exp(3*(.x-2))),col="blue") + stat_function(fun = ~ 10*(exp(5*(.x-2)))/(1+exp(5*(.x-2))),col="red") +
  labs(x='DBH (m)', y='Deathrate')+ scale_y_continuous(limits = c(0,1))

ggplot(data.frame(x = c(0, 1.5)), aes(x)) + stat_function(fun = ~ exp(0.2*.x)-1,col="blue") + stat_function(fun = ~ exp(0.5*.x)-1,col="red") +
  labs(x='DBH (m)', y='Deathrate') + theme_bw() + scale_y_continuous(limits = c(0,1))
