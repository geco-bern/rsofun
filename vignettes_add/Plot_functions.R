...
# Canopy
# Weng et al 2015
ggplot(data.frame(x = c(0, 5)), aes(x)) + stat_function(fun = ~ 0.02*(1+5*exp(4*.x-2))/(1+exp(4*.x-2)))+
  labs(x='DBH (m)', y='Deathrate')
...
#  NOW IN FORTRAN!
ggplot(data.frame(x = c(0, 5)), aes(x)) + stat_function(fun = ~ exp(-2*.x)/(0.01+exp(-2*.x)))+
  labs(x='NSC', y='Deathrate') +theme_bw()

#  Other possible option!
ggplot(data.frame(x = c(0, 5)), aes(x)) + stat_function(fun = ~ exp(-2.5*.x)/(0.01+exp(-1*.x)))+
  labs(x='NSC', y='Deathrate') +theme_bw()
...
ggplot(data.frame(x = c(0, 1)), aes(x)) + stat_function(fun = ~ 3*.x)+
  labs(x='Growth', y='Deathrate') + theme_bw()
