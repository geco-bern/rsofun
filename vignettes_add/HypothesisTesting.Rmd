...
params_species <- tibble(
  ...
  LMA            = c(0.035,   0.035,     0.035,     0.035,     0.035,   0.035,    0.035,   0.035,     0.035,     0.035,     0.035),
...
ea1sa3ma_out_hourly_tile <- out$output_hourly_tile
ea1sa3ma_out_daily_tile <- out$output_daily_tile
ea1sa3ma_out_daily_cohorts <- out$output_daily_cohorts
ea1sa3ma_out_annual_tile <- out$output_annual_tile
ea1sa3ma_out_annual_cohorts <- out$output_annual_cohorts
...
```{r}
setwd("/Users/lmarques/rsofun/Simulations_pmodel")
write.csv(ea1sa2ma_out_hourly_tile, "ea1sa2ma_out_hourly_tile.csv")
write.csv(ea1sa2ma_out_daily_tile,"ea1sca2ma_out_daily_tile.csv")
write.csv(ea1sa2ma_out_daily_cohorts,"ea1sa2ma_out_daily_cohorts.csv")
write.csv(ea1sa2ma_out_annual_tile,"ea1sa2ma_out_annual_tile.csv")
write.csv(ea1sa2ma_out_annual_cohorts,"ea1sa2ma_out_annual_cohorts.csv")
```


```{r}
ggplot() + 
  geom_line(data=ea1sa1ma_out_annual_tile[c(1:20),], aes(x=year, y=plantC, color='Control')) + 
  geom_line(data=ea1sa3ma_out_annual_tile[c(1:20),], aes(x=year, y=plantC, color='+15%')) +
  scale_color_manual("Levels of Leaf Mass Area at LUE 1", breaks = c("Control", "+15%"), values = c("#F8766D" ,"#00BA38"))+
...
mean(ea1sa1ma_out_annual_tile[c(500:1500),]$plantC)
sd(ea1sa1ma_out_annual_tile[c(500:1500),]$plantC)

mean(ea1sa2ma_out_annual_tile[c(500:1500),]$plantC)
sd(ea1sa2ma_out_annual_tile[c(500:1500),]$plantC)
...
ea1sa1ma_out_annual_cohorts <- ea1sa1ma_out_annual_cohorts %>% mutate(dbhclass = cut(dbh ,breaks = seq(0,3,0.5))) %>% mutate(basalArea = pi*(dbh/2)*density) %>% group_by(year) %>% mutate(highBA= map_dbl(basalArea,~sum(basalArea[basalArea>=.x]))-basalArea) %>%
  ungroup %>% mutate(highBAclass = cut(highBA ,breaks = seq(0,600,100)))
ea1sa2ma_out_annual_cohorts <- ea1sa2ma_out_annual_cohorts %>% mutate(dbhclass = cut(dbh ,breaks = seq(0,2.5,0.5))) %>% mutate(basalArea = pi*(dbh/2)*density) %>% group_by(year) %>% mutate(highBA= map_dbl(basalArea,~sum(basalArea[basalArea>=.x]))-basalArea) %>%
  ungroup %>% mutate(highBAclass = cut(highBA ,breaks = seq(0,600,100)))
ea1sa3ma_out_annual_cohorts <- ea1sa3ma_out_annual_cohorts %>% mutate(dbhclass = cut(dbh ,breaks = seq(0,2.5,0.5))) %>% mutate(basalArea = pi*(dbh/2)*density) %>% group_by(year) %>% mutate(highBA= map_dbl(basalArea,~sum(basalArea[basalArea>=.x]))-basalArea) %>%
  ungroup %>% mutate(highBAclass = cut(highBA ,breaks = seq(0,600,100)))
ea2sa1ma_out_annual_cohorts <- ea2sa1ma_out_annual_cohorts %>% mutate(dbhclass = cut(dbh ,breaks = seq(0,4,0.5))) %>% mutate(basalArea = pi*(dbh/2)*density) %>% group_by(year) %>% mutate(highBA= map_dbl(basalArea,~sum(basalArea[basalArea>=.x]))-basalArea) %>%
  ungroup %>% mutate(highBAclass = cut(highBA ,breaks = seq(0,600,100)))
ea2sa2ma_out_annual_cohorts <- ea2sa2ma_out_annual_cohorts %>% mutate(dbhclass = cut(dbh ,breaks = seq(0,2,0.5))) %>% mutate(basalArea = pi*(dbh/2)*density) %>% group_by(year) %>% mutate(highBA= map_dbl(basalArea,~sum(basalArea[basalArea>=.x]))-basalArea) %>%
  ungroup %>% mutate(highBAclass = cut(highBA ,breaks = seq(0,600,100)))
ea2sa3ma_out_annual_cohorts <- ea2sa3ma_out_annual_cohorts %>% mutate(dbhclass = cut(dbh ,breaks = seq(0,2,0.5))) %>% mutate(basalArea = pi*(dbh/2)*density) %>% group_by(year) %>% mutate(highBA= map_dbl(basalArea,~sum(basalArea[basalArea>=.x]))-basalArea) %>%
  ungroup %>% mutate(highBAclass = cut(highBA ,breaks = seq(0,600,100)))
ea3sa1ma_out_annual_cohorts <- ea3sa1ma_out_annual_cohorts %>% mutate(dbhclass = cut(dbh ,breaks = seq(0,4.5,0.5))) %>% mutate(basalArea = pi*(dbh/2)*density) %>% group_by(year) %>% mutate(highBA= map_dbl(basalArea,~sum(basalArea[basalArea>=.x]))-basalArea) %>%
  ungroup %>% mutate(highBAclass = cut(highBA ,breaks = seq(0,600,100)))
ea3sa2ma_out_annual_cohorts <- ea3sa2ma_out_annual_cohorts %>% mutate(dbhclass = cut(dbh ,breaks = seq(0,2.5,0.5))) %>% mutate(basalArea = pi*(dbh/2)*density) %>% group_by(year) %>% mutate(highBA= map_dbl(basalArea,~sum(basalArea[basalArea>=.x]))-basalArea) %>%
  ungroup %>% mutate(highBAclass = cut(highBA ,breaks = seq(0,600,100)))
ea3sa3ma_out_annual_cohorts <- ea3sa3ma_out_annual_cohorts %>% mutate(dbhclass = cut(dbh ,breaks = seq(0,2,0.5))) %>% mutate(basalArea = pi*(dbh/2)*density) %>% group_by(year) %>% mutate(highBA= map_dbl(basalArea,~sum(basalArea[basalArea>=.x]))-basalArea) %>%
  ungroup %>% mutate(highBAclass = cut(highBA ,breaks = seq(0,600,100)))
...
ggplot() + 
  ...
  stat_regline_equation(
    aes(label =  paste(..eq.label.., ..adj.rr.label.., sep = "~~~~")),label.x.npc = "left", label.y.npc = "top", size = 2) 
...
# Changes in WD with LUE level 1
ggplot() + 
  geom_point(data=simul_data$simuls[[2]][50:1804,], aes(x=log(QMD), y=log(Density12), color='300 kgC m-3')) + 
  geom_point(data=simul_data$simuls[[3]][50:1804,], aes(x=log(QMD), y=log(Density12), color='600 kgC m-3')) +
  geom_point(data=simul_data$simuls[[4]][50:1804,], aes(x=log(QMD), y=log(Density12), color='900 kgC m-3')) +
  ...
  theme_bw() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())
  geom_abline(intercept = coef(lm(log(Density12) ~ log(QMD), data = simul_data$simuls[[2]][50:1804,]))[1], 
              slope = coef(lm(log(Density12) ~ log(QMD), data = simul_data$simuls[[2]][50:1804,]))[2], color="#00AFBB") +
  geom_abline(intercept = coef(lm(log(Density12) ~ log(QMD), data = simul_data$simuls[[3]][50:1804,]))[1], 
              slope = coef(lm(log(Density12) ~ log(QMD), data = simul_data$simuls[[3]][50:1804,]))[2], color="#E7B800") +
  geom_abline(intercept = coef(lm(log(Density12) ~ log(QMD), data = simul_data$simuls[[4]][50:1804,]))[1], 
              slope = coef(lm(log(Density12) ~ log(QMD), data = simul_data$simuls[[4]][50:1804,]))[2], color="#FC4E07") 
...
ggplot() + 
  geom_density_ridges(data=simul_data$simuls[[12]], aes(NPP_yr,dbhclass),alpha = .8, color = "white")+
  geom_density_ridges(data=simul_data$simuls[[15]], aes(NPP_yr,dbhclass),alpha = .8, color = "black")+
  geom_density_ridges(data=simul_data$simuls[[18]], aes(NPP_yr,dbhclass),alpha = .8, color = "green")
  scale_color_manual("Levels of LUE at WD 1", breaks = c("Low", "Medium", "High"), values = c("#F8766D" ,"#00BA38", "#619CFF"))+
  labs(x = "DBH class (m)", y = expression(paste("NPP (Kg C ", m^-2, " " ,yr^-1, ") "))) + 
  theme_bw() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())


simul_data$simuls[[12]]$LevelLUE <- rep(1, dim(simul_data$simuls[[12]])[1])
simul_data$simuls[[15]]$LevelLUE <- rep(2, dim(simul_data$simuls[[15]])[1])
simul_data$simuls[[18]]$LevelLUE <- rep(3, dim(simul_data$simuls[[18]])[1])

dataBinLUE <- rbind(simul_data$simuls[[12]][,c(28,23,32)], simul_data$simuls[[15]][,c(28,23,32)], simul_data$simuls[[18]][,c(28,23,32)])

ggplot(dataBinLUE) +  geom_density_ridges(aes(NPP_yr,dbhclass,fill = paste(dbhclass, LevelLUE)),alpha = .6, color = "white")+
  labs(x = expression(paste("NPP (Kg C ", m^-2, " " ,yr^-1, ") ")), y = " DBH class (m)") +
  scale_y_discrete(expand = c(0, 0)) +  scale_x_continuous(expand = c(0, 0)) + coord_cartesian(clip = "off") + theme_bw() +
  scale_fill_cyclical(
    breaks = c("Low", "Medium", "High"),
    #labels = c(`Low` = "Low", `Medium` = "Medium"),
    values = c("#F8766D", "#F8766D", "#00BA38", "#00BA38", "#619CFF", "#619CFF"),
    name = "Option", guide = "legend"
  ) 



simul_data$simuls[[12]] <- transform(simul_data$simuls[[12]], dbhclass_num = as.numeric(dbhclass))
ggplot(data=simul_data$simuls[[12]]) + geom_density_ridges(aes(NPP_yr, dbhclass_num, group=dbhclass_num)) # x is numeric and y is factor

ggplot(data=simul_data$simuls[[18]]) + geom_density_ridges(aes(NPP_yr,dbhclass),alpha = .8, color = "white") + # x is numeric and y is factor
  labs(x = expression(paste("NPP (Kg C ", m^-2, " " ,yr^-1, ") ")), y = " DBH class (m)") +
  scale_y_discrete(expand = c(0, 0)) +  scale_x_continuous(expand = c(0, 0)) + coord_cartesian(clip = "off") + theme_bw() 
