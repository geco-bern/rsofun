...
params_modl <- list(
	...
	par_shape_tempstress = 0.0  # effectively no additional minimum temperature stress
	)
```
...
df_drivers$forcing[[1]]$tmin <- rsofun::df_drivers$forcing[[1]]$temp - 5.0 # for demo
df_drivers$forcing[[1]]$tmax <- rsofun::df_drivers$forcing[[1]]$temp + 5.0 # for demo
```
...
```{r}
ggplot() +
  geom_line(data = df_output_nostress$data[[1]], aes(x=date, y=gpp), color = 'black') + 
...
settings_calib <- list(
  ...
  maxit               = 5000, # (5 for gensa) (30 for optimr)    #
  sitenames           = "FR-Pue",
  metric              = "rmse",
  dir_results         = "./",
  name                = "BRC",
  par                 = list( kphio                 = list( lower=0.02, upper=0.07, init=0.0496 ),
                              ...
                              par_shape_tempstress  = list( lower=0.0, upper=0.2, init=0.1 ))
 )
```
