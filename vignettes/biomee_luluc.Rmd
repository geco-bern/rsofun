...
plot1 <- function(lu_name, variable, out, y_limit=NA) {
  tile <- out[[lu_name]][[1]]
...
plot_variable <- function(variable, out) {
  agg <- out[['aggregated']][[1]]
  y_limit <- max(agg[variable] * agg$lu_fraction) * 1.01

  names <- c(df_drivers$init_lu[[1]]$name, 'aggregated')

  names |> lmap(\(x) plot1(x, variable, out, y_limit))
}

plot_variables <- function(variables, out) {

  plots <- variables |> lmap(\(x) plot_variable(x, out))
...
- every 40 years, one quarter of the secondary forest will be clear-cut (but remain in the same tile).
...
df_drivers$init_cohort[[1]] <- tibble(
        ...
        lu_index            = c(1, 2)       # index land use (LU) containing this cohort. 0 (default) means all non-urban tiles will contain a copy.
)
...
harvest_transitions <- rep(harvest_pattern, 10) # We repeat the harvest pattern enough times

urban_transition <- rep(c(0, 0, 0, 0, 0, 0, 0.0006, 0, 0), n_trans)

# We add all the transition patterns
transitions <- prep(initial_transition) + prep(harvest_transitions) + prep(urban_transition)

# We set the LU definitions and transition forcing in the driver:
df_drivers$init_lu[[1]] <- lu_defs
df_drivers$luc_forcing[[1]]  <- array(transitions, c(n_lu, n_lu, n_trans))
```
...
## Land rotation and land management
