...
```{r}
load("~/rsofun/data/forcingLAE.RData")#forcingLAE See script prepare_drivers.Rmd
...
compa_inits <- df_output$data[[1]]$output_annual_tile %>% tail(500) %>% summarise(GPP = mean(GPP), LAI= quantile(LAI, probs = 0.95, na.rm=T), Density=mean(Density12), Biomass=mean(plantC)) %>% pivot_longer(everything(), names_to = "variables", values_to = "targets_mod") %>% left_join(ddf_obs)

compa_inits %>% 
  ggplot() +
  geom_point(aes(x = targets_mod, y = targets_obs, col=variables)) +
  theme_classic()+labs(x = "Predicted", y = "Observed")+
  geom_abline(col="grey") + ggtitle("DBH mortality model p-model +50% tf_base")
...
settings_calib_CST <- list(
  ...
  dir_results         = "./",
  name                = "ORG",
  par                 = list(kphio = list(lower=0.02, upper=0.1, init=0.05),
...
settings_calib_DBH <- list(
  ...
  par                 = list(kphio = list(lower=0.04, upper=0.09, init=0.05),
                             phiRL = list(lower=1, upper=6, init=3.5),
                             LAI_light = list(lower=1, upper=6, init=3.5),
                             tf_base = list(lower=0.01, upper=3, init=1),
...
settings_calib_NSC <- list(
  ...
  par                 = list(kphio = list(lower=0.04, upper=0.1, init=0.05),
                             phiRL = list(lower=1, upper=15, init=3.5),
                             LAI_light = list(lower=1, upper=12, init=3.5),
                             tf_base = list(lower=0.01, upper=6, init=0.5),
                             par_mort = list(lower=-8, upper=-1, init=-2))
 )
...
settings_calib_GR <- list(
  ...
  par                 = list(kphio = list(lower=0.03, upper=0.1, init=0.05),
                             phiRL = list(lower=1, upper=15, init=3.5),
                             LAI_light = list(lower=1, upper=15, init=3.5),
                             tf_base = list(lower=0.01, upper=3, init=0.5),
                             par_mort = list(lower=0.01, upper=4, init=1))
 )
...
load("~/data/calibrations/settings_calib_DBH.RData")
...
settings_calib_DBH_gs <- list(
  ...
  par                 = list(phiRL = list(lower=0.5, upper=5, init=3.5),
                             LAI_light = list(lower=1, upper=10, init=3.5),
                             tf_base = list(lower=0.1, upper=10, init=1),
                             par_mort = list(lower=0.01, upper=1, init=0.6))
 )
...
#### Calibrate the model gs-Leuning
...
save(settings_calib_DBH_gs, file = "~/data/forcing/settings_calib_DBH_gs.RData")
load("~/data/calibrations/settings_calib_DBH_gs.RData")
...
save(settings_calib_GR_gs, file = "~/data/forcing/settings_calib_GR_gs.RData")
load("~/data/calibrations/settings_calib_GR_gs.RData")
...
compa_inits <- df_calib_DBH_gs$data[[1]]$output_annual_tile %>%
...
compa_inits %>% 
  ...
  geom_point(aes(x = targets_mod, y = targets_obs, col=variables)) +
  theme_classic()+labs(x = "Predicted", y = "Observed")+
  geom_abline(col="grey") + ggtitle("GR mortality model gs-Leuning")
...
########## Mortality as tree size ########## 
df_drivers$params_species[[1]]$kphio  <- settings_calib_DBH$par_opt["kphio"] # Increment 15 and 30%
...
compa_inits <- df_calib_DBH$data[[1]]$output_annual_tile %>%
  summarise(GPP = mean(GPP), LAI= quantile(LAI, probs = 0.95, na.rm=T), Density=mean(Density12), Biomass=mean(plantC)) %>%  
  gather("variables", "targets_mod",GPP,LAI,Density,Biomass) %>%
  left_join(ddf_obs)
...
compa_inits %>% 
  ...
  geom_point(aes(x = targets_mod, y = targets_obs, col=variables)) +
  theme_classic()+labs(x = "Predicted", y = "Observed")+
  geom_abline(col="grey") + ggtitle("DBH mortality model p-model LAI_light +30%")
...
compa_inits<- df_calib_NSC$data[[1]]$output_annual_tile %>%
  summarise(GPP = mean(GPP), LAI= quantile(LAI, probs = 0.95, na.rm=T), Density=mean(Density12), Biomass=mean(plantC)) %>%  
  gather("variables", "targets_mod",GPP,LAI,Density,Biomass) %>%
  left_join(ddf_obs)
...
compa_inits %>% 
  ...
  geom_point(aes(x = targets_mod, y = targets_obs, col=variables)) +
  theme_classic()+labs(x = "Predicted", y = "Observed")+
  geom_abline(col="grey") + ggtitle("NSC mortality model p-model")
