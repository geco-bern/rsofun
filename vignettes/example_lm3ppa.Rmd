...
params_species <- tibble(
  ...
  alphaBM       = c(0.19,0.15,0.09,rep(0.15,13)), #In Ensheng BiomeE: 5200.0
  thetaBM       = c(2.36,2.30,2.54,rep(2.30,13)), #In Ensheng BiomeE: 2.5
...
settings_calib_DBH <- list(
  ...
  par                 = list(kphio = list(lower=0.01, upper=0.1, init=0.05),
                             phiRL = list(lower=0.1, upper=8, init=3.5),
                             LAI_light = list(lower=0.1, upper=8, init=3.5),
                             tf_base = list(lower=0.1, upper=6, init=0.5),
...
settings_calib_NSC <- list(
  ...
  name                = "ORG",
  par                 = list(kphio = list(lower=0.04, upper=0.1, init=0.05),
                             phiRL = list(lower=0.01, upper=12, init=3.5),
                             LAI_light = list(lower=0.01, upper=10, init=3.5),
                             tf_base = list(lower=0.01, upper=6, init=0.5),
                             par_mort = list(lower=-6, upper=-1, init=-2))
 )
...
settings_calib_GR <- list(
  ...
  par                 = list(kphio = list(lower=0.04, upper=0.1, init=0.05),
                             ...
                             LAI_light = list(lower=0.01, upper=10, init=3.5),
                             tf_base = list(lower=0.01, upper=6, init=0.5),
                             par_mort = list(lower=0.01, upper=3, init=1))
 )
...
load("~/data/biomass/d_param.rda")
d_param_sel <- d_param %>% dplyr::filter(equation_n==3,component_n=="Aboveground")
...
df_drivers$params_species[[1]]$LAI_light  <- settings_calib_DBH$par_opt["LAI_light"]
...
df_drivers$params_species[[1]]$LAI_light  <- settings_calib_NSC$par_opt["LAI_light"]
