...
siteinfo <- data.frame(lon = 99, lat = 99, elv = 99)
```
...
```{r}
ddf_input <- read_delim("~/BiomeE-Allocation/model/input/ORNL_forcing.txt", col_names = TRUE, delim = "\t") %>% 
...
mod <- run_lm3ppa_f_bysite( "BE-Vie", 
                            params_siml, 
                            siteinfo, 
                            ddf_input, # forcing
                            params_tile, 
