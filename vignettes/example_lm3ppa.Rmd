...
compa_all <- df_output$data[[1]]$output_annual_tile %>%
...
```{r}
ddf_obs <- tibble(GPP = mean_annual_gpp, LAI = max_annual_lai, Density= densityLAE, Biomass=Biomass_LAE) %>%
...
########## Constant Self-thinning ########## 
df_drivers$params_species[[1]]$kphio[]  <- settings_calib_CST$par_opt["kphio"]
...
########## Mortality as tree size ########## 
df_drivers$params_species[[1]]$kphio  <- settings_calib_DBH$par_opt["kphio"]
...
########## Mortality as carbon starvation ########## 
df_drivers$params_species[[1]]$kphio  <- settings_calib_NSC$par_opt["kphio"]
