...
settings_calib_DBH <- list(
  ...
  dir_results         = "./",
  name                = "ORG",
  par                 = list(kphio = list(lower=0.0, upper=0.1, init=0.085),
                             phiRL = list(lower=1, upper=8, init=3.5),
                             tf_base = list(lower=0.2, upper=5, init=1),
...
LAE_data_Fagus <- LAE_data %>% 
  dplyr::filter(SPECIES == "Fagus sylvatica")

LAE_data_Fagus <- LAE_data_Fagus %>% 
  dplyr::filter(INVYEAR == 2017)
