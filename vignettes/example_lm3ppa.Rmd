...
out$output_annual_tile %>% summarise(GPP = mean(GPP), LAI= max(LAI), NTrees=mean(Density12), Biomass=mean(plantC)) %>%  pivot_longer(everything(), names_to = "targets", values_to = "values")
...
settings_calib_CST <- list(
  ...
  par                 = list(kphio = list(lower=0.01, upper=0.07, init=0.02),
                             ...
                             tf = list(lower=0.01, upper=1.5, init=0.1),
                             CAI_max = list(lower=0, upper=4, init=0.5))
 )

# Mortality as DBH
settings_calib_DBH <- list(
  method              = "gensa", # gensa, optimr, BayesianTools, linscale
  targetvars          = c("GPP","AGB","LAI","Density"),
  timescale           = list(GPP = "y", LAI = "y", Density = "y"),
  maxit               = 5, # (5 for gensa) (30 for optimr)
  sitenames           = "CH-Lae",
  metric              = "rmse",
  dir_results         = "./",
  ...
  par                 = list(kphio = list(lower=0.01, upper=0.07, init=0.02),
                             phiRL = list(lower=1, upper=8, init=2),
                             tf = list(lower=0.01, upper=1.5, init=0.1),
                             param_dbh = list(lower=0.01, upper=1, init=0.1))
 )
...
settings_calib_NSC <- list(
  ...
  par                 = list(kphio = list(lower=0.01, upper=0.07, init=0.02),
                             phiRL = list(lower=1, upper=8, init=2),
                             tf = list(lower=0.01, upper=1.5, init=0.1),
                             param_nsc = list(lower=-3, upper=-1, init=-2.9))
 )
...
settings_calib_GR <- list(
  ...
  par                 = list(kphio = list(lower=0.01, upper=0.07, init=0.02),
                             phiRL = list(lower=1, upper=8, init=2),
                             tf = list(lower=0.01, upper=1.5, init=0.1),
                             param_gr = list(lower=0.01, upper=2, init=0.1))
 )
```
...
df_drivers_lm3ppa <- tibble(sitename,
...
settings_calib <- calib_sofun(
  df_drivers_lm3ppa = dplyr::filter(df_drivers_lm3ppa, sitename %in% settings_calib_CST$sitenames),  
  ddf_obs = ddf_obs,
