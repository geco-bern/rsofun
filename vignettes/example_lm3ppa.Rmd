...
```{r}
load("~/data/rsofun_benchmarking/calib_lm3ppa/forcingLAE.RData")#forcingLAE See script prepare_drivers.Rmd
...
```{r}
pheno_observations <- read.csv("~/data/LWF/Laegeren/pheno_data/observations/pheno_Laegeren_2021-01-27.csv", sep=";")

pheno_estimations <- read.csv("~/data/LWF/Laegeren/pheno_data/estimations/pheno_estim_LAE.csv")
...
settings_calib_CST <- list(
  ...
  par                 = list(kphio = list(lower=0.01, upper=0.07, init=0.02),
                             ...
                             par_mort = list(lower=0.1, upper=4, init=0.8))
 )
...
#### Calibrate the model
```{r}
## first test cost function xxxx
cost_rmse_lm3ppa( par, ddf_obs, df_drivers, inverse = FALSE )
