...
hhdf = 
  hhdf |>
    mutate(time = lubridate::as_datetime(TIMESTAMP_START, tz = "GMT", format="%Y%m%d%H%M")) |>
...
filn <- "../data/p_hydro_validation.rda"
message(paste0("- writing to file: ", filn))
...
load("../data/p_hydro_drivers.rda")
load("../data/p_hydro_validation.rda")
...
```{r}
output$data[[1]] %>% select(date, gpp, latenth) %>% 
  rename(le = latenth) %>% 
  melt("date") %>% 
  mutate(group="model") %>% 
  rbind(p_hydro_validation$data[[1]] %>% 
          select(date, gpp, le) %>%
          mutate(gpp = gpp*86400/1e6) %>% # convert to mol m-2 day-1
          mutate(le = le) %>% 
          melt("date") %>% 
          mutate(group="obs")) %>% 
  ggplot(aes(y=value, x=as.Date(date))) +
