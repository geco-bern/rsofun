...
# p_model_drivers_format2025_02 <- p_model_drivers
# usethis::use_data(p_model_drivers_format2025_02)  # TODO: NOT YET UPDATED FOR PHYDRO (still add default phydro_* parameters)
# p_model_validation_format2025_02 <- p_model_validation
# usethis::use_data(p_model_validation_format2025_02) 
```
...
```{r eval=FALSE}
rsofun::p_model_drivers_format2025_02$forcing[[1]] |>
    ...
    rbind(rsofun::p_model_validation_format2025_02$data[[1]] |>
            # dplyr::filter(year(date) >= ystart & year(date) <= yend) |>
    ...
    ggtitle(rsofun::p_model_drivers_format2025_02$sitename[[1]])
...
output <- rsofun::runread_pmodel_f(
  rsofun::p_model_drivers_format2025_02,
  par = params_modl
  )  
```
...
df_gpp_plot <- rbind(
  ...
  rsofun::p_model_validation_format2025_02 |>
    filter(sitename == "FR-Pue") |>
    unnest(data) |>
    select(date, gpp) |>
...
pars <- calib_sofun(
    drivers = rsofun::p_model_drivers_format2025_02,  
    obs = rsofun::p_model_validation_format2025_02,
    settings = settings,
...
# run the model for these parameters
output <- rsofun::runread_pmodel_f(
  mutate(rsofun::p_model_drivers_format2025_02,
...
df_plot <- output$data[[1]] %>% 
  ...
  left_join(
    rsofun::p_model_validation_format2025_02$data[[1]] %>% 
...
```{r}
phydro_model_drivers <- rsofun::p_model_drivers_format2025_02  |> 
