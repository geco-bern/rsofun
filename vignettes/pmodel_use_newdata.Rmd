...
params_modl <- list(
    ...
    tau_acclim         = 30.0,
    kc_jmax            = 0.41,
    whc                = rsofun::p_model_drivers_format2024_08$site_info[[1]]$whc
  )

# run the model for these parameters
output <- rsofun::runread_pmodel_f(
  rsofun::p_model_drivers_format2024_08,
  par = params_modl
  )
```
...
# run the model for these parameters
output <- rsofun::runread_pmodel_f(
  mutate(rsofun::p_model_drivers_format2024_08,
...
phdro_model_drivers$forcing_acclim[[1]] |>
    # dplyr::filter(year(date) >= ystart & year(date) <= yend) |>
    dplyr::select(date, co2, ppfd, netrad, temp, vpd, fapar, rain, ccov) |>
    tidyr::pivot_longer(-date) |>
    dplyr::mutate(type="24-hr mean") |>
    ...
    facet_wrap(~name, scales = "free")+
    ggtitle(phdro_model_drivers$sitename[[1]])
```
