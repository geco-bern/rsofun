...
Here we present an example, where we use `cost_likelihood_pmodel()` to compute the joint likelihood of all the targets specified (that is, by summing the log-likelihoods of GPP and bigD13C) and ultimately calibrate the `beta_unitcostratio` parameter.

TODO: further below we calibrate another example to multiple sites
...
saveRDS(par_calib_join, 
        here::here("vignettes", "files/param_calib_multitarget.Rmd__par_calib_join.RDS"), 
        compress = "xz")
```
...
dir.create(here::here("vignettes", "files/param_calib_multitarget/"), showWarnings = FALSE)
df_timings226 <- run_mcmc_rsofun(226, suffix_str = "setup_226",
  outpath = here::here("vignettes", "files/param_calib_multitarget/"),
  # mcmc setup:
  sampler = "DREAMzs",
  iterations = 600, 
  burnin = 0,
...
saveRDS(calib_timings,
        here::here("vignettes", "files/param_calib_multitarget.Rmd__calib_timings.RDS"),
        compress = "xz")
```
...
out_calib_226 <- readRDS(here::here("vignettes/", "files/param_calib_multitarget/calibrations/out_calib_setup_226.rds"))
out_calib_231 <- readRDS(here::here("vignettes/", "files/param_calib_multitarget/calibrations/out_calib_setup_231.rds"))
out_calib_231p<- readRDS(here::here("vignettes/", "files/param_calib_multitarget/calibrations/out_calib_setup_231_parallel.rds"))
```

```{r plot MCMC trace plot, fig.width=7, fig.height=4}
rsofun_symbol_parname_description <- tibble::tribble(
...
plot_mcmc_trace(out_calib_231, nr_internal_chains = 3, burnin_to_skip = 0, burnin_to_skip_gelman = 150)
...
p2 <- plot_prior_posterior_density(out_calib_231$mod,  burnin_to_skip = 20) +  ggtitle(out_calib_231$name)
p3 <- plot_prior_posterior_density(out_calib_231p$mod, burnin_to_skip = 20) +  ggtitle(out_calib_231p$name)
cowplot::plot_grid(p1,p2,p3, ncol = 1)
```

```{r plot parameter correlation, fig.width=7, fig.height=7}
# BayesianTools::correlationPlot(out_calib_226$mod)
# BayesianTools::correlationPlot(out_calib_231$mod)
...
predict_sofun_parallelized <- function(
  ...
  settings$n_cores <- min(nrow(par_df), 
                          settings$n_cores) # ensure not more than needed
  ...
  cl <- if (parallel_flag){
    multidplyr::new_cluster(settings$n_cores) |>
      # multidplyr::cluster_assign(
        #get_mod_obs_pmodel_bigD13C_vj_gpp = rsofun:::get_mod_obs_pmodel_bigD13C_vj_gpp
      # ) |>
...
apply_bias_correction_and_sample_error <- function(df_pred, N_sample_error){
  ...
  df_pred |>
    ...
    select(-err_sample) |> # select(, -sample_id, -err_par_bias, -err_par_sd) |>
...
out_prediction_template <- here::here("vignettes", "files/param_calib_multitarget/", 
...
# ggb <- ggplot2::ggplot_build(pl_timeseries_gpp)
# ggt <- ggplot2::ggplot_gtable(ggb)
# saveRDS(ggt,
#         "files/param_calib_multitarget/predictions/df_B3_timeseries_ggt.rds",
#         compress = "xz")
ggplot2::ggsave(
  filename = "files/param_calib_multitarget/predictions/pl_timeseries_gpp.png",
  plot = pl_timeseries_gpp, width = 7, height = 7, dpi = 150)
```
```{r eval = TRUE, include = TRUE}
# fake output since prediction was run previous to vignette rendering
# plot(readRDS("files/param_calib_multitarget/predictions/pl_scatter_comparison_ggt.rds"))
...
list_of_scenarios_to_loop_over <- list(
  # s228 = list("bigD13C" = df_228_bigD13C,  "gpp" = df_228_gpp),
...
# pl_scatter_231 <- cowplot::plot_grid(
#   nrow = 1,
#   list_of_scatters$s231$bigD13C$gg + bigD13C_labs() + labs(title = "Scenario 231"),
#   list_of_scatters$s231$gpp$gg     + gpp_labs()     + labs(title = "Scenario 231"))
# pl_scatter_231
...
pl_scatter_comparison <- cowplot::plot_grid(
  ...
  # list_of_scatters$s228$bigD13C$gg + mark_as_target + bigD13C_labs(xNULL=TRUE) ,#+ labs(title = "Scenario 228"),
  # list_of_scatters$s228$gpp$gg                      + gpp_labs(xNULL=TRUE)     ,#+ labs(title = "Scenario 228"),
  # 
  # list_of_scatters$s227$bigD13C$gg                  + bigD13C_labs(xNULL=TRUE) ,#+ labs(title = "Scenario 227"),
  # list_of_scatters$s227$gpp$gg                      + gpp_labs(xNULL=TRUE)     ,#+ labs(title = "Scenario 227"),
  # 
  # list_of_scatters$s226$bigD13C$gg + mark_as_target + bigD13C_labs(xNULL=TRUE) ,#+ labs(title = "Scenario 226"),
  # list_of_scatters$s226$gpp$gg                      + gpp_labs(xNULL=TRUE)     ,#+ labs(title = "Scenario 226"),
  # 
  # list_of_scatters$s222$bigD13C$gg                  + bigD13C_labs(xNULL=TRUE) ,#+ labs(title = "Scenario 222"),
  # list_of_scatters$s222$gpp$gg     + mark_as_target + gpp_labs(xNULL=TRUE)     ,#+ labs(title = "Scenario 222"),
  # 
  # list_of_scatters$s223$bigD13C$gg + mark_as_target + bigD13C_labs(xNULL=TRUE) ,#+ labs(title = "Scenario 223"),
  # list_of_scatters$s223$gpp$gg                      + gpp_labs(xNULL=TRUE)     ,#+ labs(title = "Scenario 223"),
...
```{r eval = FALSE, include = FALSE}
# store output since prediction isn't run when vignette is rendered
# ggb_scatter <- ggplot2::ggplot_build(pl_scatter_comparison)
# ggt_scatter <- ggplot2::ggplot_gtable(ggb_scatter)
# saveRDS(ggt_scatter,
#         "files/param_calib_multitarget/predictions/pl_scatter_comparison_ggt.rds",
#         compress = "xz")
ggplot2::ggsave(
