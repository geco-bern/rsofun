...
In this section we present an example, where we use `cost_likelihood_pmodel2()` to compute the joint likelihood of all the targets specified (that is, by summing the log-likelihoods of GPP and bigD13C) and ultimately calibrate the `beta_unitcostratio` parameter.
...
run_mcmc_rsofun <- function(
  ...
  # Load loglikelihood (e.g. if using a tailor-made one, instead of package-provided):
    ...
    # source("R/cost_likelihood_pmodel2.R", echo = FALSE)
...
During the paralell MCMC sampling the workers don't send output to the console.
...
runMCMC terminated after 40.068seconds
```
...
predict_sofun_parallelized <- function(
  ...
  run_pmodel_single_prediction <- function(par, par_fixed, drivers, obs) {
    ...
    # Taken from cost_likelihood_pmodel2()
