---
title: "Parameter calibration (P-Model) and cost functions"
author: "Josefa Arán, Fabian Bernhard"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Parameter calibration (P-Model) and cost functions}
...
The `rsofun` package allows to calibrate parameters of the `pmodel` and `biomee` models via the `calib_sofun_parallelized()` function. The implementation of the calibration is fairly flexible and can be adapted to a specific use-case via a tailor-made cost function (used as metrics for the optimization routines in `calib_sofun_parallelized()`). The package provides a set of standard cost functions named `cost_*`, which can be used for a variety of calibrations (different sets of model parameters, using various target variables, etc.). Alternatively, it's possible to write a more specific new cost function to be used together with `calib_sofun_parallelized()`.

In this vignette, we go over some examples on how to use the `rsofun` cost functions for parameter calibration with `calib_sofun_parallelized()` and how to write your own custom one from scratch.
The separate vignette 'Parameter calibration (P-Model) to multiple targets' (see `vignette("param_calib_multitarget")`) provides another example.

Two calibration approaches are showcased below. First we showcase how to find a 
single set of parameters that optimize the RMSE using a GenSA optimizer. 
Second optimizing the likelihood with a Bayesian calibration approach is shown.
...
The implementation of `cost_rmse_pmodel()` allows flexibility in various ways. We can simultaneously calibrate a subset of model parameters and also replicate the different calibration setups in Stocker et al., 2020 GMD, simply by providing the appropriate inputs to `calib_sofun_parallelized()` using arguments as `settings`, `obs` and the optional argument `par_fixed`. Note that we must always specify the values of the model parameters that aren't calibrated (via argument `par_fixed`). For example, following the `ORG` setup, only parameter `kphio` is calibrated with below code:

```{r run GenSA calibration, eval = FALSE}
...
drivers_to_use <- pmodel_drivers    |> dplyr::filter(sitename == "FR-Pue")
obs_to_use     <- pmodel_validation |> dplyr::filter(sitename == "FR-Pue")
...
pars_calib_rmse <- list(
  par = c(kphio = 0.039602577231004),
  mod = list(
    value = 1.27552392197712,
    par = c(kphio = 0.039602577231004),
    trace.mat = matrix(
      c(1, 5230, 3.56178703103092, 1.27552392197712),
      nrow = 1L,
      ncol = 4L,
...
The output of `calib_sofun_parallelized()` is a list containing the calibrated parameter values (element `par`) and the raw optimization output from the optimizer (element `mod`; here from `GenSA` or, as we see next, from `BayesianTools::runMCMC`).

Note that the standard cost functions would allow to calibrate to several targets (fluxes and leaf traits predicted by the P-model) simultaneously. 

### Calibration to GPP using a simple likelihood function and BayesianTools

Let's calibrate the parameters involved in the temperature dependency of the quantum yield efficiency, `kphio`, `kphio_par_a` and `kphio_par_b`. Taking a Bayesian calibration approach, we need to define a likelihood as cost function (we'll use `cost_likelihood_pmodel_bigD13C_vj_gpp()`). We assume that the target variable (`'gpp'`) follows a normal distribution centered at the observations and with its _unknown_ standard deviation (`'err_gpp'`), that we need to add to the calibratable parameters. We also assume a uniform prior distribution for all calibratable parameters.
By maximizing the log-likelihood, the MAP (maximum a posteriori) estimators for all 4 parameters are computed. The functions `cost_likelihood_pmodel_bigD13C_vj_gpp()` and `calib_sofun_parallelized()`, allow us to perform this type of calibration in the following way:

```{r run Bayesian calibration, eval = FALSE}
# Define calibration settings
settings_likelihood <- list(
  method = 'BayesianTools',
  metric = cost_likelihood_pmodel_bigD13C_vj_gpp,            # our cost function
...
drivers_to_use <- pmodel_drivers    |> dplyr::filter(sitename == "FR-Pue")
obs_to_use     <- pmodel_validation |> dplyr::filter(sitename == "FR-Pue")
pars_calib_likelihood <- calib_sofun_parallelized(
  # calib_sofun_parallelized arguments:
  drivers  = drivers_to_use,  
  obs      = obs_to_use,
...
saveRDS(pars_calib_likelihood, 
        "files/new_cost_function.Rmd__pars_calib_likelihood.RDS", 
        compress = "xz")
```
```{r include = FALSE}
# fake output since calibration isn't run
...
In this section, we guide you through the main ideas with an example.
To run the calibration, you can still use `calib_sofun_parallelized()` in combination with 
your custom cost function. 

The routine `calib_sofun_parallelized()` requires `drivers`, `obs` 
...
be returned.
All other (optional) arguments to `calib_sofun_parallelized()` are passed through to the cost 
...
with `calib_sofun_parallelized()` as shown below. Note that one way of developing the cost
...
pars_calib_mae <- calib_sofun_parallelized(
...
During the optimization procedure, the cost function receives as argument a suggestion of the parameters `par`. 
This might be just a subset of all needed parameters (defined via `settings$par`). 
Thus to call `runread_pmodel_f()` within the cost function, a full set of model parameters is needed. 
Here, we'll hardcode the parameters that aren't being calibrated inside the cost function. 
(Note, that in above examples they were passed as an additional argument `par_fixed`).
```{r, eval = FALSE}
cost_mae <- function(par, obs, drivers, my_own_message){
...
```{r run custom calibration, eval=FALSE}
# Define calibration settings and parameter ranges
settings_mae <- list(
  method = 'GenSA',
