...
gpp_temp_test_r <- purrr::map( as.list( seq( 0, 35, length.out = 100 ) ), 
  ~rpmodel( tc = ., vpd = 100, co2 = 300, elv = 300, kphio = params_opt$kphio, fapar = 0.7, ppfd = 10, method="wanghan", returnvar = "gpp", do_ftemp_kphio = FALSE ) 
  ) %>% unlist()
...
```{r, message=FALSE, warning=FALSE}
# params_opt <- readr::read_csv( "params_opt_kphio_soilm_global.csv	" ) ) # as an example
params_opt <- dplyr::tibble( kphio=0.0579 )
