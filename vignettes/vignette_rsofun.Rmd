...
params_opt <- read_csv( "tmpdir/params_opt_kphio_soilm_global.csv" ) # as an example
...
# ## more directly:
# source("rpmodel.R")
# out <- rpmodel( tc = 20, vpd = 100, co2 = 300, elv = 300, kphio = kphio params_opt$kphio, fapar = 0.7, ppfd = 10, method="full", returnvar = NULL )
...
## R
ptm <- proc.time()
gpp_temp_test_r <- purrr::map( as.list( seq( 0, 35, length.out = 100 ) ), ~rpmodel( tc = ., vpd = 100, co2 = 300, elv = 300, kphio = params_opt$kphio, fapar = 0.7, ppfd = 10, method="full", returnvar = "gpp" ) ) %>% unlist()
proc.time() - ptm
...
settings_sims_sitescale <- list(
  path_siteinfo   = "my_external_stuff/metainfo_fluxnet2015.csv",
  ensemble        = TRUE,
  setup           = "site",
  name            = "fluxnet2015",
