...
params_opt <- readr::read_csv( paste0( rootdir, "/tmpdir/params_opt_kphio_soilm_global.csv" ) ) # as an example
...
## R
ptm <- proc.time()
gpp_temp_test_r <- purrr::map( as.list( seq( 0, 35, length.out = 100 ) ), 
  ~rpmodel( tc = ., vpd = 100, co2 = 300, elv = 300, kphio = params_opt$kphio, fapar = 0.7, ppfd = 10, method="full", returnvar = "gpp" ) 
  ) %>% unlist()
time_R <- proc.time() - ptm

plot(  seq( 0, 35, length.out = 100 ), gpp_temp_test_r, ylim = c(0,3), type = "l", xlab = "Temperature (deg C)", ylab = "GPP (gC/d/m2)" )
lines( seq( 0, 35, length.out = 100 ), gpp_temp_test_for, col = "red" )
...
In the example given here, we use a site meta information file for FLUXNET 2015 Tier 1 sites, provided along with the `rsofun` package. It's located at the path `/rsofun/inst/extdata/siteinfo_example.csv`. For demonstration, we define a set of site-scale simulations of length one. That is, for only one site, here "FR-Pue". Note that FLUXNET 2015 data and MODIS FPAR data is provided along with the package `rsofun` for this one site for demonstration purposes only  and provides data only for a reduced number of years (2007-2014). The site meta information file looks like this:
```{r message=FALSE}
siteinfo <- readr::read_csv( paste0( rootdir, "/inst/extdata/siteinfo_example.csv" ) )
print( siteinfo )
```
...
<!-- For site-scale simulations conducted at FLUXNET 2015 Tier 1 sites, the meta information is provided along with the package `rsofun`. Let's look at this data as an example and save it in a CSV file, corresponding to the information specified above (under `settings_sims_sitescale$path_siteinfo`). -->
<!-- ```{r} -->
<!-- print( metainfo_Tier1_sites_kgclimate_fluxnet2015 ) -->
<!-- write_csv( metainfo_Tier1_sites_kgclimate_fluxnet2015, path = settings_sims_sitescale$path_siteinfo ) -->
<!-- ``` -->

```{r}
settings_sims_sitescale <- list(
  path_siteinfo   = paste0( rootdir, "/inst/extdata/siteinfo_example.csv"),
  ensemble        = TRUE,
  setup           = "site",
  name            = "example",
  dir_sofun       = "~/alphadata01/bstocker/sofun/trunk/",
  path_output     = "~/alphadata01/bstocker/sofun/output_example_sofun/",
  path_output_nc  = "~/alphadata01/bstocker/sofun/output_nc_example_sofun/",
  path_input      = "~/alphadata01/bstocker/sofun/input_example_sofun/",
  grid            = NA,
  implementation  = "fortran",
  in_ppfd         = TRUE,
  in_netrad       = FALSE,
...
settings_input_sitescale <-  list(
  ...
  splined_fapar            = TRUE,
  path_co2                 = paste0( rootdir, "/inst/extdata/cCO2_rcp85_const850-1765.dat" ),
  path_fluxnet2015         = paste0( rootdir, "/inst/extdata/" ),
  path_MODIS_FPAR_MCD15A3H = paste0( rootdir, "/inst/extdata/" )
  )
```
...
settings_calib <- list(
  ...
  path_fluxnet2015 = paste0( rootdir, "/inst/extdata/" ),
  path_gepisat     = paste0( rootdir, "/inst/extdata/" ),
  maxit            = 50,
  ...
  metric           = "rmse",
  dir_results      = paste0( rootdir, "/tmpdir" )
 )
```
...
```{r, message=FALSE, warning=FALSE}
params_opt <- readr::read_csv( paste0( settings_calib$dir_results, "/params_opt_example.csv" ) ) # as an example
...
settings_eval <- list(
  ...
  agg = 5,
  path_fluxnet2015_d = paste0( rootdir, "/inst/extdata/" ),
  path_fluxnet2015_w = "",
  path_fluxnet2015_m = "",
  path_fluxnet2015_y = "",
  path_gepisat_d     = ""
...
```{r, eval=TRUE, message=FALSE, warning=FALSE}
modobs_daily <- out_eval$plotfunctions$plot_modobs_daily()
```
