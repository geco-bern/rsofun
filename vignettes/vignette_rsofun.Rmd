...
params_opt <- read_csv( paste0( rootdir, "/tmpdir/params_opt_kphio_soilm_global.csv" ) ) # as an example
...
```{r message=FALSE}
siteinfo <- read_csv( paste0( rootdir, "/inst/extdata/siteinfo_example.csv" ) )
print( siteinfo )
```
...
```{r, eval=FALSE, message=FALSE, warning=FALSE}
params_opt <- read_csv( paste0( settings_calib$dir_results, "/params_opt_example.csv" ) ) # as an example
...
settings_eval <- list(
  ...
  path_fluxnet2015_m = "",
  path_fluxnet2015_y = "",
  path_gepisat_d     = paste0( rootdir, "/inst/extdata/" )
  )
```
