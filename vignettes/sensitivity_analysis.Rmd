...
For simplicity, we plot the predicted GPP using the MAP estimate
for all calibrated parameters with a 90% credible interval based on the assumption that the
...
plot_gpp_data <- runread_pmodel_f(
    drivers = p_model_drivers,
    par =  list(                       
  ...
  dplyr::left_join(p_model_validation$data[[1]] |>
                     dplyr::rename(gpp_obs = gpp) |>
                     dplyr::select(date, gpp_obs), 
                   by = 'date')
