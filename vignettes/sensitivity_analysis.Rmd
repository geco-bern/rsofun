...
According to the previous sensitivity analysis, calibrating the error parameter for GPP and the quantum yield efficiency parameters will have the most impact on the model fit. Let's run the calibration:
```{r eval = evaluate}
set.seed(2023)
...
# The same as above but for kc_jmax only. It's easier to calibrate less parameters at a time.
set.seed(2023)
...
In addition to visualizations, it's helpful to compute some convergence diagnostics, like the Gelman-Brooks-Rubin (GBR) "potential scale reduction factor" (psrf). This diagnostic compares the variance within chains to that across chains and should progressively get closer to 1. It is common in the literature to accept a GBR between 1.05 and 1.1, indicating convergence. We use the `gelman.diag` function from the `{coda}` library to compute the psrf (on the MCMC values, having removed the burnin), and we can see below that it hasn't converged yet.

```{r eval = evaluate}
coda::gelman.diag(par_calib$mod$chain, multivariate = FALSE)
```

Finally, the parameter MAP estimates can be derived from the chains (that converged) after removing the burnin period. These and other statistics are shown with `summary(par_calib$mod)`.
