...
GPP predictions from the P-model and GPP observations at the site `FR-Pue`, 
...
ll_pmodel <- function(
  ...
  rsofun::cost_likelihood_pmodel( # reuse likelihood cost function
    as.list(par_v),
    obs     = rsofun::pmodel_validation |> filter(sitename == "FR-Pue"),
    drivers = rsofun::pmodel_drivers    |> filter(sitename == "FR-Pue"),
...
# saveRDS(morrisOut, "vignettes/files/sensitivity_analysis.Rmd__morrisOut.RDS")
morrisOut <- readRDS("files/sensitivity_analysis.Rmd__morrisOut.RDS")  # NOTE: for rendering remove prefix "vignettes/"
```
...
morrisOut.df |>
  ...
  ggplot(aes(
    ...
    value,
    fill = variable)) +
  geom_bar(position = position_dodge(), stat = "identity", color = NA) +
  scale_fill_manual("",
...
It is always important to check the convergence of the MCMC algorithm used for the Bayesian calibration. 
Here we show some plots and statistics that may help you assess whether the parameter calibration has converged.

According to the previous sensitivity analysis, calibrating the error parameter for GPP and the quantum yield efficiency parameters will have a high impact on the model fit. 
Let's run the calibration:
```{r eval = FALSE, echo = TRUE}
...
settings_calib <- list(
  ...
  control = list(
    ...
    settings_runMCMC = list(
      burnin = 12000,
      iterations = 24000,
      nrChains   = 3,# number of independent chains to be sampled
      startValue = 3 # number of internal chains to be sampled (dependent)
    ),
    n_parallel_nrChains = 1 # do not parallelize the three independent chains
  ),
  par = list(
    ...
    soilm_thetastar = list(
      lower = 0.01 * rsofun::pmodel_drivers$site_info[[5]]$whc,
      upper = 1.0  * rsofun::pmodel_drivers$site_info[[5]]$whc,
      init  = 0.6  * rsofun::pmodel_drivers$site_info[[5]]$whc
    ),
...
saveRDS(par_calib, "vignettes/files/sensitivity_analysis.Rmd__par_calib.RDS", compress = "xz")
...
par_calib <- readRDS("files/sensitivity_analysis.Rmd__par_calib.RDS") # NOTE: for rendering remove prefix "vignettes/"
```
...
run_pmodel <- function(par) {
  ...
  out <- runread_pmodel_f(
    # use same drivers as for calibration
    drivers = pmodel_drivers |> dplyr::filter(sitename == "FR-Pue"),
    # use calibrated parameters and fixed parameters
    par =  list(
      kphio              = par$kphio,
      ...
      beta_unitcostratio = par_fixed$beta_unitcostratio, # 146.0,
      rd_to_vcmax        = par_fixed$rd_to_vcmax, # 0.014,
      tau_acclim         = par_fixed$tau_acclim, # 20.0,
      kc_jmax            = par_fixed$kc_jmax  # 0.41
    )
  )
}
run_pmodel_add_error <- function(par) {
  # Function that runs the P-model for a sample of parameters
...
pmodel_runs <- samples_par |>
  dplyr::mutate(sim = purrr::map(pars, ~ run_pmodel_add_error(.x))) |>
...
pmodel_run_MAP <- run_pmodel_add_error(
...
# fake output since calibration isn't run
# saveRDS(pmodel_runs, file = "vignettes/files/sensitivity_analysis.Rmd__pmodel_runs.RDS")
pmodel_runs <- readRDS("files/sensitivity_analysis.Rmd__pmodel_runs.RDS")  # NOTE: for rendering remove prefix "vignettes/"
```
...
data_to_plot <- pmodel_runs |>
  ...
  dplyr::left_join(
    ...
    p_model_oldformat_validation$data[[1]][1:365, ] |>
...
data_to_plot <- pmodel_run_MAP |>
  ...
  dplyr::left_join(
    # Merge GPP validation data (first year)
    pmodel_validation |> filter(sitename == "FR-Pue") |>
      unnest(data) |> slice(1:365) |>
      dplyr::rename(gpp_obs = gpp),
    by = "date")
