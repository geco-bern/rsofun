...
their physical meaning and the site FR-Pue 
...
plot_acf_mcmc <- function(chains, par_names){
  ...
  if(!all(par_names %in% chains[[1]][[1]]$names)){
    stop(sprintf("Requested par_names: %s,\navailable par_names: %s", 
                 paste0(par_names, collapse=","),
                 paste0(chains[[1]][[1]]$names, collapse=",")))
  }
  for(par_name in par_names){
    for(i in 1:n_chains){
      stopifnot(all(par_names %in% chains[[i]][[1]]$names)) # check again
...
plot_acf_mcmc(
  par_calib$mod, 
  par_names = c("kphio", "kc_jmax", "soilm_thetastar", "err_gpp"))
```
...
run_pmodel_with_error <- function(par){
  # Function that runs the P-model for a sample of parameters
...
pmodel_runs <- samples_par |>
  dplyr::mutate(sim = purrr::map(pars, ~run_pmodel_with_error(.x))) |>
...
plot_gpp_error <- ggplot(data = data_to_plot) +
  ...
  geom_ribbon(
    aes(x = date,
        ymin = gpp - 2*BayesianTools::MAP(par_calib$mod)$parametersMAP['err_gpp'], 
        ymax = gpp + 2*BayesianTools::MAP(par_calib$mod)$parametersMAP['err_gpp'],
  ...
  scale_color_manual(NULL, aesthetics = c("colour","fill"),
                     breaks = c("Observations",
                                "Predictions based on MAP",
                                "90% confidence interval"),
                     values = c("black", "tomato", "grey40"))
plot_gpp_error
```
