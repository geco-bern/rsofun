...
Statistic $\mu *$ and $\sigma$ measure the average of the absolute
differences between these log-likelihood values and their standard deviation,
respectively. The higher the value of these statistics, the more influential they 
are 
```{r eval = evaluate}
...
cost_likelihood <- function(
  ...
  # get observation in the same format
  obs <- obs |>
    dplyr::select(sitename, data) |>
    tidyr::unnest(data)
  
  # left join obs and output
  ...
  ll <- sum(
    stats::dnorm(
      df[['gpp_mod']],                 # simulated GPP
      mean = df[['gpp']],              # observed GPP
      sd = par[2],                         # error term
      log = TRUE
    ))  
...
params_new$kphio <- par_calib$par[1]
...
We can observe the convergence of the MCMC below. The quantum yield efficiency
clearly converged quite quickly
```{r eval = evaluate}
plot(par_calib$mod)
```
...
For PEPA:
Let's take a look at the different sources of error in the model, focusing on Vcmax25 as target.
```{r eval = evaluate}
validation_data <- p_model_validation %>%
  tidyr::unnest(data) |>
