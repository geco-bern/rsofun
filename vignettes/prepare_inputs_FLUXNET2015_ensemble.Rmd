...
The following describes how to use the [ingestr R package ](https://stineb.github.io/ingestr/) for collecting the forcing and evaluation data for all FLUXNET 2015 Tier 1 sites used in Stocker et al. (2020). Files are available on Euler XXXpathXXX.
...
ddf_forcing <- ddf %>%
  dplyr::select(sitename, date, temp = temp_day_fluxnet2015, prec = prec_fluxnet2015, vpd = vpd_day_fluxnet2015, ppfd = ppfd_fluxnet2015, patm = patm_fluxnet2015,
         fapar = fapar_spl)   # missing: ccov, co2
ddf_obs <- ddf %>% 
...
```{r}
save(df_drivers_fluxnet2015, file = "~/data/stocker20gmd_outputs/df_drivers_fluxnet2015.Rdata")
```
