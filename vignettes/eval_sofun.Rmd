...
if (!file.exists(filn)){
  ...
  siteinfo_eval <- siteinfo$light %>% dplyr::rename( sitename = mysitename ) %>% left_join( meta, by = "sitename")
  ...
  siteinfo_eval <- siteinfo_eval %>% mutate( kgnumber = raster::extract( kgclass, data.frame( x=.$lon, y=.$lat ) ) ) %>% 
...
if (!file.exists("mod_RED.Rdata")){
  ...
  params_opt <- read_csv( "../results/params_opt_RED.csv" )
...
if (!file.exists("obs_eval_NT.Rdata")){
  obs_eval_NT <- get_obs_eval( settings_eval = settings_eval, settings_input = settings_input_sitescale, settings_sims = settings_sims_sitescale, overwrite = TRUE )
  save( obs_eval_NT, file = "obs_eval_NT.Rdata")
} else {
  load("obs_eval_NT.Rdata")
}

## evaluate RED simulations
if (!exists("out_eval_RED.Rdata")){
  out_eval_RED <- eval_sofun( mod_RED, settings_eval, settings_sims_sitescale, siteinfo, obs_eval = obs_eval_NT, doplot=FALSE, overwrite = TRUE )
  save( out_eval_RED, file = "out_eval_RED.Rdata" )
} else {
  load("out_eval_RED.Rdata")
}

row_rsqtable_RED <- bind_cols( Setup = "RED", getrow_statstable( out_eval_RED, stat = "rsq" ) )

## Plot mod-vs-obs for daily values
# if (!exists("out_eval_RED")) load("out_eval_RED.Rdata")
...
if (!file.exists("mod_FULL.Rdata")){
  ...
  ## update files containing parameter values read by model
  params_opt <- read_csv( "../results/params_opt_FULL.csv" )
  nothing <- update_params( params_opt, settings_sims_sitescale$dir_sofun )
  ...
  save( mod_FULL, file="mod_FULL.Rdata" )
} else {
  load("mod_FULL.Rdata")
}
...
if (!file.exists("out_eval_FULL.Rdata")){
  out_eval_FULL <- eval_sofun( mod_FULL, settings_eval, settings_sims_sitescale, siteinfo, obs_eval = obs_eval_NT, doplot=FALSE, overwrite = TRUE )
  save( out_eval_FULL, file = "out_eval_FULL.Rdata" )
} else {
  load("out_eval_FULL.Rdata")
}
...
if (!file.exists("out_eval_NULL.Rdata")){
  out_eval_NULL <- eval_sofun( mod_NULL, settings_eval, settings_sims_sitescale, siteinfo, obs_eval = obs_eval_NT, doplot=FALSE, overwrite = TRUE )
  save( out_eval_NULL, file = "out_eval_NULL.Rdata" )
} else {
  load("out_eval_NULL.Rdata")
}
...
## evaluate NULL simulations
if (!file.exists("out_eval_NULLpft.Rdata")){
  out_eval_NULLpft <- eval_sofun( mod_NULLpft, settings_eval, settings_sims_sitescale, siteinfo, obs_eval = obs_eval_NT, doplot=FALSE, overwrite = TRUE )
  save( out_eval_NULLpft, file = "out_eval_NULLpft.Rdata" )
} else {
  load("out_eval_NULLpft.Rdata")
}
...
if (!file.exists("mod_RED_FPARitp.Rdata")){
  ...
  ## update files containing parameter values read by model
  params_opt <- read_csv( "../results/params_opt_RED_FPARitp.csv" )
  nothing <- update_params( params_opt, settings_sims_sitescale$dir_sofun )
  ...
  save( mod_RED_FPARitp, file="mod_RED_FPARitp.Rdata" )
} else {
  load("mod_RED_FPARitp.Rdata")
}
...
if (!file.exists("out_eval_RED_FPARitp.Rdata")){
  out_eval_RED_FPARitp <- eval_sofun( mod_RED_FPARitp, settings_eval, settings_sims_sitescale, siteinfo, obs_eval = obs_eval_NT, doplot=FALSE, overwrite = TRUE )
  save( out_eval_RED_FPARitp, file = "out_eval_RED_FPARitp.Rdata" )
} else {
  load("out_eval_RED_FPARitp.Rdata")
}

row_rsqtable_RED_FPARitp <- bind_cols( Setup = "RED_FPARitp", getrow_statstable( out_eval_RED, stat = "rsq" ) )
```
...
if (!file.exists("mod_RED_EVI.Rdata")){
  ...
  ## update files containing parameter values read by model
  params_opt <- read_csv( "../results/params_opt_RED_EVI.csv" )
  nothing <- update_params( params_opt, settings_sims_sitescale$dir_sofun )
  ...
  save( mod_RED_EVI, file="mod_RED_EVI.Rdata" )
} else {
  load("mod_RED_EVI.Rdata")
}
...
if (!file.exists("out_eval_RED_EVI.Rdata")){
  out_eval_RED_EVI <- eval_sofun( mod_RED_EVI, settings_eval, settings_sims_sitescale, siteinfo, obs_eval = obs_eval_NT, doplot=FALSE, overwrite = TRUE )
  save( out_eval_RED_EVI, file = "out_eval_RED_EVI.Rdata" )
} else {
  load("out_eval_RED_EVI.Rdata")
}
...
## run the model with these parameters and read output into mod_RED_EVI
if (!file.exists("mod_RED_DT.Rdata")){
  ...
  ## update files containing parameter values read by model
  params_opt <- read_csv( "../results/params_opt_RED_DT.csv" )
  nothing <- update_params( params_opt, settings_sims_sitescale$dir_sofun )
  ...
  save( mod_RED_DT, file="mod_RED_DT.Rdata" )
} else {
  load("mod_RED_DT.Rdata")
}
...
## get observational data separately and save since it's the same for multiple evaluations
if (!file.exists("obs_eval_DT.Rdata")){
  obs_eval_DT <- get_obs_eval( settings_eval = settings_eval, settings_input = settings_input_sitescale, settings_sims = settings_sims_sitescale, overwrite = TRUE )
  save( obs_eval_DT, file = "obs_eval_DT.Rdata")
} else {
  load("obs_eval_DT.Rdata")
}

## evaluate RED_DT simulations
if (!file.exists("out_eval_RED_DT.Rdata")){
  out_eval_RED_DT <- eval_sofun( mod_RED_DT, settings_eval, settings_sims_sitescale, siteinfo, obs_eval = obs_eval_DT, doplot=FALSE, overwrite = TRUE )
  save( out_eval_RED_DT, file = "out_eval_RED_DT.Rdata" )
} else {
  load("out_eval_RED_DT.Rdata")
}
...
## run the model with these parameters and read output into mod_RED_EVI
if (!file.exists("mod_RED_Ty.Rdata")){
  ...
  ## update files containing parameter values read by model
  params_opt <- read_csv( "../results/params_opt_RED_Ty.csv" )
  nothing <- update_params( params_opt, settings_sims_sitescale$dir_sofun )
  ...
  save( mod_RED_Ty, file="mod_RED_Ty.Rdata" )
} else {
  load("mod_RED_Ty.Rdata")
}
...
## get observational data separately and save since it's the same for multiple evaluations
if (!file.exists("obs_eval_Ty.Rdata")){
  obs_eval_Ty <- get_obs_eval( settings_eval = settings_eval, settings_input = settings_input_sitescale, settings_sims = settings_sims_sitescale, overwrite = TRUE )
  save( obs_eval_Ty, file = "obs_eval_Ty.Rdata")
} else {
  load("obs_eval_Ty.Rdata")
}

## evaluate RED_Ty simulations
if (!file.exists("out_eval_RED_Ty.Rdata")){
  out_eval_RED_Ty <- eval_sofun( mod_RED_Ty, settings_eval, settings_sims_sitescale, siteinfo, obs_eval = obs_eval_Ty, doplot=FALSE, overwrite = TRUE )
  save( out_eval_RED_Ty, file = "out_eval_RED_Ty.Rdata" )
} else {
  load("out_eval_RED_Ty.Rdata")
}
