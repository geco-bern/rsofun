...
df_modis_fpar <- df_modis_fpar %>%
  mutate(
    data = purrr::map(data, ~rename(., fapar = modisvar_filled)) # *** not clear if wanted to rename to fapar or to modisvar_filled (the column is already called fapar)
    )
```
