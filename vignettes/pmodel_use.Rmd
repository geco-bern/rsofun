---
title: "P-model usage"
author: "Koen Hufkens, Josefa Arán, Fabian Bernhard, Benjamin Stocker"
...
Here we give a short example on how to run the `p-model` on the included demo dataset to familiarize yourself with both the data structure and the outputs.

## Demo data

The package includes a demo dataset to run and validate p-model output using GPP or D13C observations. These files can be directly loaded into your workspace by typing:
...
These are real data from eight fluxnet sites (CH-Dav, CZ-BK1, DK-Sor, FI-Hyy, FR-Pue, GF-Guy, US-Ha1, US-PFa) 
and twelve sites with with leaf carbon isotope signatures D13C [(Cornwell, 2025)](https://doi.org/10.5281/zenodo.15239220). 
Information about data sources 
and information about data structure, variable names, and their meaning and units can be found in the reference pages of `pmodel_drivers` and `pmodel_validation`.
We can use these data to run the model, together with observations of GPP or D13C (`pmodel_validation`) we can also calibrate `p-model` parameters.
Since leaf traits are only measured once per site, the forcings used are average climate conditions during the growing seasons.

~~For the remainder of this vignette, we will use the GPP fluxnet datasets. The workflow is exactly the same for leaf traits data.~~
...
# Define model drivers.
driver <- rsofun::pmodel_drivers |>
    # subset only certain sites:
    dplyr::filter(sitename %in% c(
      # i.e. only use sites where we request runs of the daily p-model (internally
      # calling `run_pmodel_f_bysite`, and not `run_pmodel_onestep_f_bysite`)
      "FR-Pue",
      "CH-Dav", "CZ-BK1", "DK-Sor", "FI-Hyy", "GF-Guy", "US-Ha1", "US-PFa"))

# Run the model for these parameters and drivers.
output <- rsofun::runread_pmodel_f(driver, par = params_modl)
```
...
df_gpp_plot <- output |>
  tidyr::unnest(data) |>
  dplyr::select(sitename, date, gpp_mod = gpp) |>
  dplyr::left_join(
    rsofun::pmodel_validation |> dplyr::filter( sitename %in% c(
      "FR-Pue",
      "CH-Dav", "CZ-BK1", "DK-Sor", "FI-Hyy", "GF-Guy", "US-Ha1", "US-PFa")) |>
      tidyr::unnest(data) |>
  ...
  # Plot only first year of each site
  group_by(sitename) |> dplyr::slice(1:365)
...
pars <- calib_sofun_parallelized(
  drivers = pmodel_drivers |> filter(sitename == "FR-Pue"),
  obs     = pmodel_validation |> filter(sitename == "FR-Pue"),
  settings = settings,
  # extra arguments passed to the cost function:
  par_fixed = params_modl[-1]  # fix non-calibrated parameters to previous 
                               # values, removing kphio [-1]
  )
```
...
output_new <- rsofun::runread_pmodel_f(
  pmodel_drivers |> filter(sitename == "FR-Pue"),
  par = params_modl
  )

# Update data.frame for plotting
df_gpp_plot_FRPue <- df_gpp_plot |> 
  select(-starts_with("gpp_calib")) |>
  left_join(
    output_new |>
      unnest(data) |>
      select(sitename, date, gpp_calib = gpp),
    by = c("sitename", "date")
  ) |>
  filter(sitename == "FR-Pue")

# Plot GPP
ggplot(data = df_gpp_plot_FRPue) +
  geom_point(
    aes(
      x = date,
      y = gpp_obs,
