...
Another two datasets are provided as an example to validate the model against leaf traits data, rather than fluxes. Measurements of Vcmax25 (aggregated over species) for several sites from Atkin et al. 2015 are given in `p_model_validation_vcmax25` and the corresponding forcing for the pmodel is given in `p_model_drivers_vcmax25`. Since leaf traits are only measured once per site, the forcing is a single year of average climate (for each day of the year).
...
For the remainder of this vignette, we will use the flux datasets. The workflow is exactly the same for leaf trait data. 
