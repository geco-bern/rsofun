...
```{r}
# changes for making rwsc run
p_model_drivers$params_siml[[1]]$in_netrad <- FALSE
# p_model_drivers$site_info[[1]] <- p_model_drivers$site_info[[1]] |> 
#   rename(rzwsc = whc)
p_model_drivers <- p_model_drivers |> 
  mutate(forcing = purrr::map(forcing, ~mutate(., netrad = 80.0)))
...
settings <- list(
  ...
  par = list(
    ...
    soilm_par_a = list(lower=0.0,  upper=1.0, init = 0.05),
    soilm_par_b = list(lower=0.0,  upper=2.0, init = 0.05)
    )
)
```
...
params_modl$soilm_par_a <- pars$par["soilm_par_a"]
params_modl$soilm_par_b <- pars$par["soilm_par_b"]
