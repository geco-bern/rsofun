...
# this is to deal with an error p_model_oldformat_drivers.rds not being found 
p_model_oldformat_drivers

p_model_oldformat_validation
```

These are real data from the French FR-Pue fluxnet site. Information about data structure, variable names, and their meaning and units can be found in the reference pages of `p_model_oldformat_drivers` and `p_model_oldformat_validation`. We can use these data to run the model, together with observations of GPP we can also calibrate `p-model` parameters.
...
output <- rsofun::runread_pmodel_f(
  rsofun::pmodel_drivers |>
    ...
    dplyr::filter(sitename %in% c(
      "FR-Pue",
      "CA-Cbo", "CH-Aws", "CH-Fru", "ES-LM2", "FI-Let",
      "IT-Lav", "IT-MBo", "IT-Noe", "IT-Ro2", "US-SRM"
    )) |>
    # only use sites where we request runs of the daily p-model (internally
...
df_gpp_plot <- output |>
  ...
  dplyr::left_join(
    rsofun::pmodel_validation |> dplyr::filter(run_model == "daily") |>
      tidyr::unnest(data) |>
