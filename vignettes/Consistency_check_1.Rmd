...
path_orig_L  <- "/Users/lmarques/BiomeE-Allocation/model"
path_sofun_L <- "/Users/lmarques/sofun"
...
```{r, message=FALSE, include=T, eval=T}
...
plot_dfs <- lapply(names(biomeE_out_daily_cohorts),function(nm)data.frame(col1 = biomeE_out_daily_cohorts[,nm], col2 = sofun_out_daily_cohorts[,nm]))
...
varnams_annual_sofun <- names(biomeE_out_annual_tile)[c(-1, -length(names(biomeE_out_annual_tile)))]
all.equal(varnams_annual_biomeE, varnams_annual_sofun)
for (varnam in varnams_annual_biomeE[1:10]){
  print(varnam)
...
sofun_out_annual_cohorts <- sofun_out_annual_cohorts[1:30626,][,-24]
...
for (idx in seq_along(plot_dfs))
  print(ggplot()+geom_smooth(data = plot_dfs[[idx]], aes(x=col1, y=col2),color="blue") +
      geom_smooth(data = plot_dfs[[idx]], aes(x=col1, y=col2),color="red")+
