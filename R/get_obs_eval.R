...
get_obs_eval <- function( settings_eval, settings_sims, overwrite = TRUE ){
	...
	if ("gpp" %in% names(settings_eval$benchmark)){
	  ...
	  if ("fluxnet2015" %in% datasource){
		  ...
		  if (!file.exists("tmpdir/adf_obs_eval.Rdata")||overwrite){
			  ...
			  save( adf, file = "tmpdir/adf_obs_eval.Rdata")
	    } else {
	    	load("tmpdir/adf_obs_eval.Rdata")
	    }
			...
			if (!file.exists("tmpdir/mdf_obs_eval.Rdata")||overwrite){
				...
				save( mdf, file = "tmpdir/mdf_obs_eval.Rdata")
			} else {
	    	load("tmpdir/mdf_obs_eval.Rdata")
	    }
			...
			if (!file.exists("tmpdir/ddf_obs_eval.Rdata")||overwrite){
          ...
          ## add to other data frames (ddf, adf, mdf) and take take weighted average for updated 'gpp_obs'
				...
				save( ddf, file = "tmpdir/ddf_obs_eval.Rdata")

			} else {
	    	load("tmpdir/ddf_obs_eval.Rdata")
	    }
		  ...
		  ##------------------------------------------------------------
			if (!file.exists("tmpdir/xdf_obs_eval.Rdata")||overwrite){
			  ...
			  ## Generate vector of starting dates of X-day periods, making sure the 1st of Jan is always the start of a new period
				...
				save( xdf, breaks_xdf = xdf, file = "tmpdir/xdf_obs_eval.Rdata")	 
			} else {
				load("tmpdir/xdf_obs_eval.Rdata")
			}
	}
