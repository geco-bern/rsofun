...
run_pmodel_f_bysite <- function(
  ...
  if ("nyeartrend" %in% names(params_siml)) {
    stop("Unexpectedly received params_siml$nyeartrend for p-model.")
  }
  ...
  if ("firstyeartrend" %in% names(params_siml)) {
    stop("Unexpectedly received params_siml$firstyeartrend for p-model.")
  }
...
build_out_pmodel <- function(pmodelout, firstyeartrend, nyeartrend) {
  ...
  if (all(is.na(pmodelout))) {
    pmodelout <- array(dim = c(1, 21), data = NA_real_)
  }
