...
#' Creates a joint log-likelihood cost function for several setups
...
create_cost_joint_likelihood_pmodel <- function(
    params_modl,
    setup
){
  # predefine variables for CRAN check compliance
  ...
  if(any(is.null(names(obs)), is.null(names(drivers)))){
    error('The obs and drivers lists must be named,
              with the target variable names.')
  } else if(! all(names(obs) == names(drivers))){
    error('The obs and drivers list names must coincide.')
  }
  
  # loop over targets
  ll <- sapply(names(obs), function(target){
    # run the model
    df <- runread_pmodel_f(
      drivers[[target]], 
      par = params_modl,
    ...
    df <- df %>%
      ...
      tidyr::unnest(data)
    # rename column
    colnames(df)[which(colnames(df) == target)] <- paste0(target, '_mod')
    ...
    # left join with observations
    if('date' %in% colnames(obs)){
      df <- dplyr::left_join(df, df_obs, by = c('sitename', 'date'))
    } else {
      df <- dplyr::left_join(df, df_obs, by = c('sitename')) 
    }
    
    # get observations and predicted target values, without NA            
    df <- df[, c(paste0(target, '_mod'), target, paste0(target, '_unc'))] |>
      tidyr::drop_na()
