...
FluxForcing <- FluxForcing %>% dplyr::rename(PAR=PPFD_IN,Swdown=SW_IN_F,TEMP=TA_F,SoilT=TS_F_MDS_1,RH=RH,RAIN=P_F,WIND=WS_F,PRESSURE=PA_F,SWC=SWC_F_MDS_1) %>% dplyr::select(-TIMESTAMP_START) %>% 
  mutate(date = make_datetime(YEAR, MONTH, DOY, HOUR))  %>%
  dplyr::filter(!(mday(date)==29 & month(date)==2)) %>% mutate(DOY=lubridate::yday(date)) %>% dplyr::select(-MONTH) #%>% drop_na()
# Hourly data
FluxForcing <- FluxForcing %>% 
    dplyr::group_by(lubridate::year(date),lubridate::month(date),lubridate::day(date),lubridate::hour(date)) %>% 
    summarise_at(vars(1:13), list(~mean(., na.rm = TRUE)))  %>% mutate(sitename = "CH-Lae") 
...
if (params_siml$method_photosynth == "gs_leuning"){
  FluxForcing <- FluxForcing %>% 
    ...
    summarise_at(vars(1:12), list(~mean(., na.rm = TRUE)))
  FluxForcing <- FluxForcing[,-c(1:3,16)]
}
