...
#'
#' @param siteinfo A character string specifying the path to the site meta 
...
prepare_setup_sofun <- function(siteinfo, params_siml){
  ...
  if (is.character(siteinfo)){
    if (!file.exists(siteinfo)){
      stop( "prepare_setup_sofun():
            Path specified by siteinfo does not exist." )
    } 
    siteinfo <- utils::read.csv2( siteinfo )
  }
  ...
  siteinfo <- siteinfo %>% 
    ...
    mutate(date_start = lubridate::ymd( 
             paste0( as.character( siteinfo$year_start ), "-01-01" ) ),
           date_end   = lubridate::ymd( 
             paste0( as.character( siteinfo$year_end ), "-12-31" ) ))
