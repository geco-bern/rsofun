...
#'       \item{LMAmin}{Minimum LMA, leaf mass per unit area, LMAmin/LMA describes fraction of leaf that become fine litter (metabolic parts), kg C m\eqn{^{-2}}. Has to be smaller than LMA. Difference to LMA is structural parts (becoming coarse litter).}
...
#'       \item{gamma_LN}{Leaf respiration coefficient per unit N, in molC kgN\eqn{^{-1}} yr\eqn{^{-1}}.}
...
#'       \item{kphio}{Quantum yield efficiency \eqn{\varphi_0} at optimal temperature, in mol mol\eqn{^{-1}}. (optional, defaults to 0.05)}
#'       \item{beta}{Unit cost ratio of carboxylation to transpiration, unitless. (optional, defaults to 146.0)}
#'       \item{rd_to_vcmax}{Ratio of Rdark to Vcmax25, unitless. (optional, defaults to 0.014)}
#'       \item{kc_jmax}{Jmax cost ratio, unitless. (optional, defaults to 0.41)}
#'       \item{kphio_par_a}{Shape parameter of temperature-dependency of quantum yield efficiency, in degC\eqn{^{-2}}. (optional, defaults to 0.0)}
#'       \item{kphio_par_b}{Optimal temperature of quantum yield efficiency, in degC. (optional, defaults to 25.0)}
...
#'       \item{alphaCA}{Coefficient for allometry (projected crown area = alphaCA * DBH_m ** thetaCA), in m\eqn{^{2}} \eqn{^{-/thetaCA}}.}
#'       \item{thetaCA}{Coefficient for allometry (projected crown area = alphaCA * DBH_m ** thetaCA), unitless. Dybzinski (eq. G1) showed that thetaCA = thetaBM - 1. Moreover, Weng et al. (2015, eq A2) uses thetaBM = thetaHT + 2.}
...
#'       \item{extinct}{TODO: same as kappa below) light extinction coefficient in the canopy (Beer's law), unitless. (optional, defaults to 0.75)}
#'       \item{kappa}{light extinction coefficient in the canopy (Beer's law), unitless. (optional, defaults to 0.5)}
...
#'       \item{A_mort}{A coefficient in understory mortality rate correction, (deathrate = mortrate_d_u * (1+A*exp(B*DBH))/(1+exp(B*DBH))), in year\eqn{^{-1}}. (optional, defaults to 9.0)}
#'       \item{B_mort}{B coefficient in understory mortality rate correction, (deathrate = mortrate_d_u * (1+A*exp(B*DBH))/(1+exp(B*DBH))), in m\eqn{^{-1}}. (optional, defaults to -60.0)}
#'     }}\item{The following columns pertaining to the \bold{leaf parameters}:}{\describe{
#'       \item{LMA}{Leaf mass per unit area (kg C m\eqn{^{-2}}).}
#'       \item{LNbase}{Basal leaf N per unit leaf area, in kg N m\eqn{^{-2}}. NOTE: it is used to represent the N content in metabolic parts of the leaf. (TODO: correct?)}
#'       \item{CNleafsupport}{C/N ratio of the structural parts of the leaf (non-metabolic parts).}
#'       \item{tauNSC}{Residence time of C in NSC (to define storage capacity), in years.}
#'       \item{f_LFR_max}{Max fraction of total C growth that is allocated to leaves and fine root (remaining C growth used for seeds and DBH growth), unitless. (optional, defaults to 0.85)}#'       \item{fNSmax}{Multiplier for NSNmax as sum of potential bl and br.}
#'       \item{internal_gap_frac}{Fraction of internal gaps in the canopy}
#'     }}\item{The following columns pertaining to the \bold{C/N ratios for plant pools}:}{\describe{
#'       \item{CNsw0}{C/N ratio of sapwood, in kg C kg N\eqn{^{-1}}.}
#'       \item{CNwood0}{C/N ratio of heartwood, in kg C kg N\eqn{^{-1}}.}
#'       \item{CNroot0}{C/N ratio of roots, in kg C kg N\eqn{^{-1}}.}
#'       \item{CNseed0}{C/N ratio of seeds, in kg C kg N\eqn{^{-1}}.}
...
#'       \item{init_cohort_age}{Initial tree/cohort age, in yr. (optional)
#'        Default: 0.}
