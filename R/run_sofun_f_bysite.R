...
run_sofun_f_bysite <- function( settings_sims, params_modl, list_soiltexture, forcing ){
  ...
  ## convert forcing data frame to matrix
  forcing <- forcing %>% 
  ...
  if (any(is.nanull(forcing$temp))){
    rlang::abort(paste("Error: Missing value in temp for site", settings_sims$sitename))
    cont <- FALSE
  }
  if (any(is.nanull(forcing$rainf))){
    rlang::abort(paste("Error: Missing value in rainf for site", settings_sims$sitename))
    cont <- FALSE
  }
  if (any(is.nanull(forcing$vpd))){
    rlang::abort(paste("Error: Missing value in vpd for site", settings_sims$sitename))
    cont <- FALSE
  }
  if (any(is.nanull(forcing$ppfd))){
    rlang::abort(paste("Error: Missing value in ppfd for site", settings_sims$sitename))
    cont <- FALSE
  }
  if (any(is.nanull(forcing$netrad))){
    rlang::abort(paste("Error: Missing value in netrad for site", settings_sims$sitename))
    cont <- FALSE
  }
  if (any(is.nanull(forcing$fsun))){
    rlang::abort(paste("Error: Missing value in fsun for site", settings_sims$sitename))
    cont <- FALSE
  }
  if (any(is.nanull(forcing$snowf))){
    rlang::abort(paste("Error: Missing value in snowf for site", settings_sims$sitename))
    cont <- FALSE
  }
  if (any(is.nanull(forcing$co2))){
    rlang::abort(paste("Error: Missing value in co2 for site", settings_sims$sitename))
    cont <- FALSE
  }
  if (any(is.nanull(forcing$ndep))){
    rlang::abort(paste("Error: Missing value in ndep for site", settings_sims$sitename))
    cont <- FALSE
  }
  if (any(is.nanull(forcing$fapar))){
    rlang::abort(paste("Error: Missing value in fapar for site", settings_sims$sitename))
    cont <- FALSE
  }
