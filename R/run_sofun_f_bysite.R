...
run_sofun_f_bysite <- function( settings_sims, params_modl, list_soiltexture, forcing ){
  ...
  forcing <- forcing %>% 
    dplyr::mutate(temp = temp_day, rainf = prec, vpd = vpd_day, netrad = -9999.9, fsun = (100-ccov_cru_int)/100, snowf = 0.0, ndep = 0.0) %>% 
    dplyr::select(temp, rainf, vpd, ppfd, netrad, fsun, snowf, co2, ndep, fapar) %>% 
    # dplyr::filter(is.na(temp) | is.na(rainf) | is.na(vpd) | is.na(ppfd) | is.na(fsun) | is.na(co2) | is.na(fapar))
    as.matrix()
