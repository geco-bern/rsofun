  ...
  subroutine pmodel_f(         &
    ...
    do yr=1,myinterface%params_siml%runyears
      ...
      if (yr > myinterface%params_siml%spinupyears ) then
        ...
        output(idx_start:idx_end,19) = dble(out_biosphere%latenth_soil(:))
        output(idx_start:idx_end,20) = dble(out_biosphere%dpsi(:))
        output(idx_start:idx_end,21) = dble(out_biosphere%psi_leaf(:))
