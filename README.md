...
This is the "centre-piece" that is used by all scripts that loop over sites in a given simulation suite. It may also be generated by the R script in repository getin `prepare_paramfils_<simsuite>.R`, available in repository [getin](https://bitbucket.org/labprentice/getin). It must (at least) contain the following columns:
...
flexible for cases where CO2 is manipulated per site/experiment.
```r
setwd("your_local_path/getin")
source("get_co2.R")
```
