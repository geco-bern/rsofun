...
settings_sims <- list( 
  ...
  path_output     = "~/sofun/output_fluxnet2015_sofun/s19/",
  path_output_nc  = "~/sofun/output_nc_fluxnet2015_sofun/s19/",
...
settings_calib <- list(
  ...
  par              = list( kphio = list( lower=0.01, upper=0.12, init=0.04 ) ),
  method           = "optimr",
  targetvars       = c("gpp"),
  datasource       = list( gpp = c("fluxnet2015", "gepisat") ),
  timescale        = list( gpp = "d" ),
  path_fluxnet2015 = "~/data/FLUXNET-2015_Tier1/20160128/point-scale_none_1d/original/unpacked/",
  path_gepisat     = "~/data/gepisat/v3_fluxnet2015/daily_gpp/",
  maxit            = 15,
  sitenames        = dplyr::filter(siteinfo$light, c4 %in% c(FALSE, NA) )$mysitename, # calibrate for non-C4 sites
  filter_temp_min  = 5.0,
...
```{r message=FALSE}
mylist <- read_csv("myselect_fluxnet2015.csv") %>% dplyr::filter( use==1 ) %>% select( -use ) %>% unlist()
settings_eval <- list(
